#!/usr/bin/env python3
"""
Convert RTF files from IE11249 (Kunkhyen Gorampa Collection) to TEI XML format.

This script converts RTF files that already contain Unicode (MonlamUniOuChan2 font)
...
Input structure:
    toprocess/IE11249-VE3KG292/GRAM000.rtf
    toprocess/IE11249-VE3KG292/GRAM001_དཀར་ཆགས།.rtf
    ...

Output structure:
    IE11249_OUTPUT/archive/VE3KG292/UT3KG292_0001.xml
    IE11249_OUTPUT/archive/VE3KG292/UT3KG292_0002.xml
    IE11249_OUTPUT/sources/VE3KG292/GRAM000.rtf
    ...
...
OUTPUT_DIR = BASE_DIR / "IE11249_OUTPUT"
...
def get_volume_folders() -> list:
    ...
    Each folder is named like: IE11249-VE3KG292
    ...
    for folder in TOPROCESS_DIR.iterdir():
        if folder.is_dir() and folder.name.startswith(f'{IE_ID}-'):
            ve_id = folder.name.replace(f'{IE_ID}-', '')  # "VE3KG292"
...
def get_ut_id(ve_id: str, file_index: int) -> str:
    ...
    VE3KG292, index 0 -> UT3KG292_0001
    VE3KG292, index 1 -> UT3KG292_0002
    """
    ve_suffix = ve_id[2:]  # Remove 'VE' prefix -> "3KG292"
...
def convert_rtf_to_tei(rtf_path: Path, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    The RTF files already contain Unicode text (MonlamUniOuChan2 font),
    so we just need to parse and normalize - no Dedris conversion needed.
    
    Args:
        rtf_path: Path to RTF file
        ve_id: Volume Entity ID (e.g., "VE3KG292")
        ut_id: Unit Text ID (e.g., "UT3KG292_0001")
        src_path: Source path for XML header (e.g., "sources/VE3KG292/GRAM000.rtf")
    ...
    # Clean up: remove other common RTF field remnants
    body_content = re.sub(r':Rare Texts of the Sakya TraditionRa\s*', '', body_content)
    body_content = re.sub(r'Texts of the Sakya Tradition - text TEMPLATE\.dotx\s*', '', body_content)
    body_content = re.sub(r'\bAcrock\b\s*', '', body_content)
...
if __name__ == "__main__":
    ...
    DEBUG_MODE = False
    DEBUG_VOLUME = "VE3KG292"  # First volume for testing
