...
# RTF \u format: \uN followed by optional single digit (replacement count), then that many
# replacement chars. N is 16-bit signed (max 5 digits). We must not consume the count digit
# as part of N (e.g. \u0FB11 means codepoint 0x0FB1, then skip 1 replacement).
_RE_UNICODE = re.compile(r'\\u(-?\d{1,5})(\d)?')
    ...
    def parse_file(self, file_path, show_progress=True):
        ...
        while i < data_len:
            ...
            if c == '{':
                ...
                # Handle \uN[d] unicode: N = codepoint (16-bit signed), optional d = replacement count
                ...
                if m:
                    ...
                    try:
                        ...
                        text_parts.append('?')
                    i = m.end()
                    # Skip replacement character(s). Count from optional digit after N.
                    skip_count = int(m.group(2)) if m.group(2) else 0
                    if skip_count == 0 and i < data_len and data[i] == '?':
                        skip_count = 1  # common RTF pattern \uN? (literal ?)
                    for _ in range(skip_count):
                        if i >= data_len:
                            break
                        m2 = _RE_HEX_CHAR.match(data, i)
                        if m2:
                            i = m2.end()
                        else:
                            i += 1  # skip one literal replacement character
                    continue
