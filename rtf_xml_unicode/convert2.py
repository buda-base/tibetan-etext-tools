...
Input structure:
    ...
    Example: IE1KG4310/toprocess/IE1KG4310-VE1KG4310/volume-0001/*.rtf
...
Usage:
    ...
    python convert.py /path/to/IE1KG4310

    # Process only a specific VE_ID:
    python convert.py --ve-id VE1KG4310_001

    # Process with explicit IE_ID override:
    python convert.py /path/to/folder --ie-id IE1KG4310
"""
...
from tibetan_text_fixes import (
    ...
    fix_question_mark_placeholders,
    fix_hi_tag_spacing,
    count_tibetan_chars,
)
...
def get_volume_folders(ie_id: str, toprocess_dir: Path) -> list:
    ...
    Each subfolder is named {IE_ID-VE_ID}, containing volume folders (e.g., volume-0001).
    ...
    for folder in toprocess_dir.iterdir():
        ...
        # Folder name is IE_ID-VE_ID (e.g. IE1KG4310-VE1KG4310)
        name = folder.name
        ve_id = name.split("-", 1)[1] if "-" in name else name

        # Look for volume subfolders (e.g., volume-0001, volume-0002)
        volume_folders = [vf for vf in folder.iterdir() if vf.is_dir() and not vf.name.startswith('.')]
        
        if volume_folders:
            # Process each volume folder
            for vol_folder in volume_folders:
                rtf_files = list(vol_folder.glob("*.rtf"))
                if rtf_files:
                    volumes.append((ve_id, vol_folder.name, vol_folder, None))
        else:
            # Fallback: check for RTF files directly in IE_ID-VE_ID folder (old structure)
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    for item in converted_streams:
        ...
        if ENABLE_NORMALIZATION:
            ...
            # Fix '?' placeholders (RTF \\u replacement misparse or Dedris conversion artifacts)
            fixed_text = fix_question_mark_placeholders(fixed_text)
...
def process_volume(ie_id: str, ve_id: str, volume_id: str, volume_folder: Path, 
    ...
    try:
        ...
        # Include volume folder in sources path if volume_id exists
        if volume_id:
            sources_output_dir = output_dir / "sources" / folder_id / volume_id
        else:
        ...
        for file_index, rtf_file in enumerate(rtf_files):
            try:
                ...
                # Build source path for metadata (includes volume folder if present)
                if volume_id:
                    src_path = f"{folder_id}/{volume_id}/{rtf_file.name}"
                else:
