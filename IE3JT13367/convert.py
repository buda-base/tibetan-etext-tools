...
INPUT_DIR = Path(r"/Users/tenzinmonlam/Documents/dharmaduta/file_convert_2")
...
def discover_collections(input_dir: Path) -> list:
    ...
    Returns:
        List of (ie_id, toprocess_dir, output_dir, sources_dir) tuples
    """
    collections = []
    ...
    for ie_folder in input_dir.iterdir():
        ...
        if nested_path.exists():
            ...
            sources_dir = ie_folder / "sources"
            collections.append((ie_id, toprocess_dir, output_dir, sources_dir))
            continue
...
def remove_page_markers(text: str) -> str:
    """
    Remove page markers including:
    - Simple: '- PAGE 10 -', 'PAGE 11', etc.
    - Complex: 'PAGE«གྲཨཀྱཎགླཁ¿དྷཱི།页：དྷཱི།ཁདྷཱི། དྷཱི།¿»'
    """
    # Remove simple PAGE markers with numbers
    text = re.sub(r'-*\s*PAGE\s+\d+\s*-*', '', text, flags=re.IGNORECASE)

    # Remove specific unwanted patterns
    text = text.replace('གྲཨཀྱཎགླཁ', '')
    text = text.replace('དྷཱི།ཁདྷཱི།', '')
    text = text.replace('དྷཱི།', '')
    text = text.replace('页：', '')
    text = text.replace('PAGE«¿¿»', '')

    # Remove PAGE at the end of text
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    for stream in streams:
        ...
        cleaned_text = remove_page_markers(cleaned_text)
    ...
    body_content = remove_page_markers(body_content)
    body_content = re.sub(r'\n\s*\n', '\n', body_content)
...
def process_volume(args: tuple) -> dict:
    ...
    Args:
        args: Tuple of (ie_id, ve_id, volume_folder, output_dir, input_sources_dir)
        
    Returns:
        dict with results: {ie_id, ve_id, success, failed, errors}
    """
    ie_id, ve_id, volume_folder, output_dir, input_sources_dir = args
    ...
    try:
        ...
        # Copy any .doc files from input sources directory
        if input_sources_dir and input_sources_dir.exists():
            doc_files = list(input_sources_dir.glob("*.doc"))
            for doc_path in doc_files:
                try:
                    dest_doc = sources_dir / doc_path.name
                    shutil.copy2(doc_path, dest_doc)
                except Exception as e:
                    result["errors"].append(f"Failed to copy {doc_path.name}: {str(e)}")
...
def process_collection(ie_id: str, toprocess_dir: Path, output_dir: Path, input_sources_dir: Path, workers: int) -> tuple:
    """
    ...
    work_items = [
        (ie_id, ve_id, volume_folder, output_dir, input_sources_dir)
...
def process_all_collections(input_dir: Path, workers: int, ie_filter: str = None):
    ...
    if ie_filter:
        collections = [(ie_id, tp, out, src) for ie_id, tp, out, src in collections if ie_id == ie_filter]
        if not collections:
    ...
    for idx, (ie_id, toprocess_dir, output_dir, input_sources_dir) in enumerate(collections):
        logger.info(f"\n[{idx + 1}/{len(collections)}] Processing {ie_id}")
        logger.info(f"  Input: {toprocess_dir}")
        logger.info(f"  Output: {output_dir}")
        
        success, failed = process_collection(ie_id, toprocess_dir, output_dir, input_sources_dir, workers)
