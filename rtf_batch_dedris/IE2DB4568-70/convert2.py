...
Run this of sub folder contain volume folder
Input structure:
    {IE_ID}/toprocess/{IE_ID-VE_ID}/{VOL_ID}/*.rtf
    Example: IE1KG22691/toprocess/IE1KG22691-VE3KG192/volume-001/*.rtf

Output structure:
    Archive (flat): {IE_ID}_output/archive/{VE_ID}/UT{suffix}_{FILE_NUM}.xml
    Sources (nested): {IE_ID}_output/sources/{IE_ID-VE_ID}/{VOL_ID}/*.rtf 
Usage:
    # Process all IE_ID-VE_ID / VOL_ID folders in current directory (auto-detects IE_ID):
    python convert2.py

    # Process specific directory (auto-detects IE_ID):
    python convert2.py /path/to/IE1KG22691

    # Process only a specific VE_ID:
    python convert2.py --ve-id VE3KG192

    # Process with explicit IE_ID override:
    python convert2.py /path/to/folder --ie-id IE1KG22691
"""
...
def get_volume_folders(ie_id: str, toprocess_dir: Path) -> list:
    ...
    Structure: {IE_ID}/toprocess/{IE_ID-VE_ID}/{VOL_ID}/*.rtf
    Each subfolder of toprocess is {IE_ID-VE_ID}; under that, each {VOL_ID} (e.g. volume-001) contains *.rtf.
    ...
    for folder in toprocess_dir.iterdir():
        ...
        # Folder name is IE_ID-VE_ID (e.g. IE1KG22691-VE3KG192)
        name = folder.name
        ve_id = name.split("-", 1)[1] if "-" in name else name

        # Each subfolder is VOL_ID (e.g. volume-001) containing *.rtf
        for vol_folder in folder.iterdir():
            if not vol_folder.is_dir() or vol_folder.name.startswith('.'):
                continue
            rtf_files = list(vol_folder.glob("*.rtf"))
            if rtf_files:
                volume_id = vol_folder.name
                volumes.append((ve_id, volume_id, vol_folder, None))
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    for item in converted_streams:
        ...
        # Skip PAGE header/footer artifacts (PAGE, PAGE 1, PAGE PAGE PAGE 4, 112PAGE 111, etc.)
        stripped = fixed_text.strip()
        if (re.fullmatch(r'PAGE\s*\d*', stripped) or re.fullmatch(r'\d+PAGE\s*\d+', stripped)
                or re.fullmatch(r'(?:PAGE\s*){2,}\d*', stripped) or stripped.startswith('PAGE PAGE ')):
            continue
    ...
    # Remove double inverted question marks; collapse double left guillemets to single; remove $$
    body_content = body_content.replace('¿¿', '')
    body_content = body_content.replace('««', '«')
    body_content = body_content.replace('$$', '')
    body_content = body_content.replace('»»', '»')
    body_content = body_content.replace('·', '')
    body_content = body_content.replace('Ã', '')
    body_content = body_content.replace('\u00b6', '')  # Remove ¶ (U+00B6)
    # Fix misread digit: 46པ -> 460 (Tibetan པ after digits should be 0)
    body_content = re.sub(r'(\d+)པ', r'\g<1>0', body_content)
    # Fix stray ASCII period (.) in Tibetan context: often failed Dedris conversion for ད (U+0F51)
    # e.g. .ེ་ -> དེ་, ཐ.་ -> ཐད་, .ོན་ -> དོན་, མེ. -> མེད
    body_content = re.sub(r'(?<=[\u0f00-\u0fff])\.|\.(?=[\u0f00-\u0fff])', '\u0f51', body_content)
    # Remove PAGE header/footer artifacts (112PAGE 111, PAGE PAGE 1<, PAGE PAGE PAGE 4, etc.)
    body_content = re.sub(r'\d+PAGE\s*\d+', '', body_content)  # e.g. 112PAGE 111
    body_content = re.sub(r'(?:PAGE\s*){2,}\d*', '', body_content)  # PAGE PAGE PAGE 4, etc.
    body_content = re.sub(r'PAGE\s*PAGE\s*\d+<', '', body_content)
    body_content = re.sub(r'\bPAGE\s*\d*\b', '', body_content)
    ...
    # Merge consecutive same-rend <hi> blocks so one tag wraps the whole run (e.g. head across line breaks)
    for rend in ('head', 'small'):
        pattern = re.compile(r'</hi>\s*(?:<lb/>\s*)*<hi rend="' + rend + r'">')
        while True:
            new_content = pattern.sub('<lb/>', body_content)
    ...
    # Collapse consecutive spaces (TEI/schema often disallow "consecutive spaces")
...
def process_volume(ie_id: str, ve_id: str, volume_id: str, volume_folder: Path,
                   output_dir: Path, collection_name: str, start_file_index: int = 0) -> tuple:
    """
    ...
    Args:
        ...
        start_file_index: Starting file index for UT ID (for flat archive across volumes)

    Returns:
        tuple: (result dict, next_file_index)
    """
    ...
    try:
        ...
        if not rtf_files:
            return result, start_file_index

        logger.info(f"  Processing {volume_label}: {len(rtf_files)} RTF files")

        # Archive: flat under archive/{VE_ID}/; sources: nested under sources/{IE_ID-VE_ID}/{VOL_ID}/
        folder_id = f"{ie_id}-{ve_id}"
        archive_dir = output_dir / "archive" / ve_id
        sources_output_dir = output_dir / "sources" / folder_id / (volume_id or "")

        archive_dir.mkdir(parents=True, exist_ok=True)
        sources_output_dir.mkdir(parents=True, exist_ok=True)

        # Process each RTF file (file_index is global across volumes for this VE_ID for flat archive)
        for file_index, rtf_file in enumerate(rtf_files):
            try:
                # Generate UT ID
                ut_id = get_ut_id(ve_id, start_file_index + file_index)

                # Build source path for metadata
                src_path = f"{folder_id}/{volume_id}/{rtf_file.name}" if volume_id else f"{folder_id}/{rtf_file.name}"
        ...
        next_file_index = start_file_index + len(rtf_files)
...
def process_collection(ie_id: str, base_dir: Path, ve_id_filter: str = None) -> dict:
    ...
    # Process each volume (track file index per VE_ID for flat archive naming)
    total_success = 0
    total_failed = 0
    next_file_index_by_ve = {}

    for ve_id, volume_id, volume_folder, collection_name in volumes:
        start_index = next_file_index_by_ve.get(ve_id, 0)
        result, next_index = process_volume(
            ie_id, ve_id, volume_id, volume_folder, output_dir, collection_name,
            start_file_index=start_index
        )
        next_file_index_by_ve[ve_id] = next_index
...
def main():
    ...
    parser.add_argument(
        ...
        nargs='?',
        default=None,
        help="Base directory containing toprocess folder (e.g., /path/to/IE1KG22691)"
    )
    ...
    if not toprocess_dir.exists():
        ...
        logger.error(f"Expected structure: {base_dir}/toprocess/{{IE_ID-VE_ID}}/{{VOL_ID}}/*.rtf")
        return
