...
def parse_derge_file(file_path: str) -> dict:
    ...
    for line in content.split('\n'):
        ...
        if line_match:
            ...
            # Extract Derge milestones {D###} or {D###a} from line content
            milestone_matches = re.findall(r'\{(D\d+[a-z]*(?:-\d+)?)\}', line_content)
...
def process_annotations(text: str) -> str:
    ...
    # Process Derge milestones {D###}, {D###a}, {D###-#}, and also malformed
    # cases where the closing } is absent (source data bug): match the ID and
    # consume the } if present, otherwise stop at the first non-ID character.
    result = re.sub(
        r'\{(D\d+[a-z]*(?:-\d+)?)(?:\}|(?=[^}a-z0-9-]))',
    ...
    # Do not escape content that may already be XML from an inner substitution.
