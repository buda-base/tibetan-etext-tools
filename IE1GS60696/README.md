...
- `normalization.py` - Unicode normalization with Tibetan character ordering
- `tibetan_text_fixes.py` - Tibetan-specific text fixes
- `GTER_TSHEG_FIX.md` - Technical documentation for vowel alignment fixes
...
### Tibetan Vowel Alignment Fixes (Latest)
- **Punctuation Mark Reordering**: Fixed alignment of Tibetan punctuation marks with vowels
  - **GTER TSHEG (༔)**: Now correctly appears after vowels (e.g., `ལོ༔` instead of `ལ༔ོ`)
  - **Shad marks (།༎༏༐༑)**: Now correctly appear after vowels (e.g., `མོ།` instead of `མ།ོ`)
  - Added new `PostVowelMark` category in Unicode normalization system
  - Updated character categorization for U+0F0D-0F11 (shad marks) and U+0F14 (GTER TSHEG)
  - Implemented cross-stream normalization to handle cases where marks and vowels are split across RTF streams
  - See `GTER_TSHEG_FIX.md` for detailed technical documentation
...
- Enhanced Unicode normalization with proper Tibetan character ordering
