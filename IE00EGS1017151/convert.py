#!/usr/bin/env python3
"""
Convert RTF files from IE00EGS1017151 (Sa-kya Kabum Collection) to TEI XML format.
...
Input structure:
    toprocess/IE00EGS1017151-VE3KG253/SKYA000.rtf
    toprocess/IE00EGS1017151-VE3KG253/SKYA001.rtf
    ...

Output structure:
    archive/VE3KG253/UT3KG253_0001.xml
    archive/VE3KG253/UT3KG253_0002.xml
    sources/VE3KG253/SKYA000.rtf
    ...
...
IE_ID = "IE00EGS1017151"

# Paths
BASE_DIR = Path(r"C:\Users\GANGA GYATSO\OneDrive\Documents\IE1PD100944\IE00EGS1017151")
TOPROCESS_DIR = BASE_DIR / "IE00EGS1017151" / "toprocess"
OUTPUT_DIR = BASE_DIR / "IE00EGS1017151_output"
...
def get_volume_folders() -> list:
    ...
    Each folder is named like: IE00EGS1017151-VE3KG253
    ...
    for folder in TOPROCESS_DIR.iterdir():
        if folder.is_dir() and folder.name.startswith(f'{IE_ID}-'):
            ve_id = folder.name.replace(f'{IE_ID}-', '')  # "VE3KG253"
...
def get_ut_id(ve_id: str, file_index: int) -> str:
    ...
    VE3KG253, index 1 -> UT3KG253_0002
    """
    ve_suffix = ve_id[2:]  # Remove 'VE' prefix -> "3KG268"
...
def convert_rtf_to_tei(rtf_path: Path, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    Args:
        ...
        ve_id: Volume Entity ID (e.g., "VE3KG253")
        ut_id: Unit Text ID (e.g., "UT3KG253_0001")
        src_path: Source path for XML header (e.g., "sources/VE3KG253/SKYA000.rtf")
...
if __name__ == "__main__":
    ...
    DEBUG_MODE = False
    DEBUG_VOLUME = "VE3KG253"  # First volume for testing
