#!/usr/bin/env python3
"""Convert intermediate XML files from IE21888 to TEI XML format. (Type C)"""
    ...
    if SOURCE_DIR.exists():
        for folder in SOURCE_DIR.iterdir():
            if folder.is_dir() and folder.name.startswith('VE'):
                ...
                volumes.append({'ve_id': folder.name, 'folder_path': folder, 'source_xmls': source_xmls})
                logger.info(f"  Found volume {folder.name} with {len(source_xmls)} source XMLs")
...
def convert_all():
    ...
    if count == 0: logger.error("No files"); return
    
    success = 0
    for i in range(count):
        v, x = volumes[i], xml_files[i]
        ve_id = v['ve_id']
        arch_dir, src_dir = OUTPUT_DIR/"archive"/ve_id, OUTPUT_DIR/"sources"/ve_id
        arch_dir.mkdir(parents=True, exist_ok=True); src_dir.mkdir(parents=True, exist_ok=True)
        try:
            for src_xml in v['source_xmls']: shutil.copy2(src_xml, src_dir/src_xml.name)
            shutil.copy2(x, src_dir/x.name)
            ut_id = get_ut_id(ve_id)
            with open(arch_dir/f"{ut_id}.xml", 'w', encoding='utf-8') as f:
                f.write(convert_xml_to_tei(x, ve_id, ut_id, f"sources/{ve_id}/{x.name}"))
            logger.info(f"Processed {ve_id}"); success += 1
        except Exception as e: logger.error(f"Error {ve_id}: {e}")
