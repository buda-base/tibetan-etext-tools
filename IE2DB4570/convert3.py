...
Input structure:
    {IE_ID}/toprocess/{IE_ID-VE_ID}/KHADON/*.rtf
    Example: IE1KG4310/toprocess/IE1KG4310-VE1KG4310_001/KHADON/*.rtf

Output structure:
    Archive (flat): {IE_ID}_output/archive/{IE_ID-VE_ID}/UT{suffix}_{FILE_NUM}.xml
...
Usage:
    ...
    python convert3.py /path/to/IE1KG4310

    # Process only a specific VE_ID:
    python convert3.py --ve-id VE1KG4310_001

    # Process with explicit IE_ID override:
    python convert3.py /path/to/folder --ie-id IE1KG4310
"""
...
def get_volume_folders(ie_id: str, toprocess_dir: Path) -> list:
    """
    Get list of RTF folders from toprocess directory.

    Structure: {IE_ID}/toprocess/{IE_ID-VE_ID}/KHADON/*.rtf
    Each subfolder of toprocess is {IE_ID-VE_ID}; under that, KHADON contains *.rtf.

    Returns:
        List of (ve_id, volume_id, rtf_folder_path, collection_name) tuples.
        volume_id is None (single KHADON folder per IE_ID-VE_ID).
    """
    volumes = []
    ...
    for folder in toprocess_dir.iterdir():
        ...
        # RTF files are in KHADON subfolder
        khadon = folder / "KHADON"
        if khadon.is_dir():
            rtf_files = list(khadon.glob("*.rtf"))
            if rtf_files:
                volumes.append((ve_id, None, khadon, None))
...
def process_volume(ie_id: str, ve_id: str, volume_id: str, volume_folder: Path, 
    ...
    try:
        ...
        # Output paths: archive (flat) and sources (nested) under {IE_ID-VE_ID} only
        folder_id = f"{ie_id}-{ve_id}"
        archive_dir = output_dir / "archive" / folder_id
        ...
        for file_index, rtf_file in enumerate(rtf_files):
            try:
                ...
                # Build source path for metadata (no volume subfolder)
...
def main():
    ...
    if not toprocess_dir.exists():
        ...
        logger.error(f"Expected structure: {base_dir}/toprocess/{{IE_ID-VE_ID}}/KHADON/*.rtf")
        return
