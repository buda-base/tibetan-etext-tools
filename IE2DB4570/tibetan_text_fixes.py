...
def fix_guillemet_bracket_pairs(text: str) -> str:
    """
    Normalize «(«…«TIBETAN1«··«TIBETAN2«)» to («TIBETAN1»TIBETAN2).

    Input example:  «(«¶¶«ཞ«··«ཚིག་«)»
    Output example: («ཞ»ཚིག་)

    Tibetan values may differ; the bracket structure is normalized the same way.
    """
    if not text:
        return text
    # Tibetan block U+0F00–U+0FFF; match one or more Tibetan chars
    tibetan_run = r'[\u0f00-\u0fff]+'
    pattern = (
        r'«\(«'           # «(«
        r'.*?'             # optional filler (e.g. ¶¶, «¶¶«)
        r'«(' + tibetan_run + r')«··«(' + tibetan_run + r')«\)»'
    )
