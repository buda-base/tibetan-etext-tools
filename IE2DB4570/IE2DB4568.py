...
Input structure:
    ...
    Example: IE2DB4568/toprocess/IE1KG4310-VE1KG4310_001/*.rtf
...
Usage:
    ...
    python IE2DB4568.py /path/to/IE1KG4310

    # Process only a specific VE_ID:
    python IE2DB4568.py --ve-id VE1KG4310_001

    # Process with explicit IE_ID override:
    python IE2DB4568.py /path/to/folder --ie-id IE2DB4568
"""
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    for item in converted_streams:
        ...
        # Skip PAGE header/footer artifacts (PAGE, PAGE 1, 112PAGE 111, PAGE PAGE 1<, etc.)
        stripped = fixed_text.strip()
        if (re.fullmatch(r'PAGE\s*\d*', stripped) or stripped.startswith('PAGE PAGE ')
                or re.fullmatch(r'\d+PAGE\s*\d+', stripped)):
            continue
    ...
    body_content = body_content.replace('¶¶', '¶')
    body_content = body_content.replace('··', '·')
    # Remove PAGE header/footer artifacts (PAGE, PAGE 1, 112PAGE 111, PAGE PAGE 1<, etc.)
...
def process_volume(ie_id: str, ve_id: str, volume_id: str, volume_folder: Path, 
    ...
    try:
        ...
        # Output paths use IE_ID-VE_ID folder name
