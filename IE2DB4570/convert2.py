...
Output structure:
    Archive (flat): {IE_ID}_output/archive/{IE_ID-VE_ID}/{VOL_ID}/UT{suffix}_{FILE_NUM}.xml
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    body_content = body_content.replace('$$', '')
    body_content = body_content.replace('»»', '»')
    body_content = body_content.replace('ÃÃ', '')
    body_content = body_content.replace('¶¶', '¶')
    body_content = body_content.replace('··', '·')
    # Normalize «(«…«TIBETAN1«··«TIBETAN2«)» → («TIBETAN1»TIBETAN2)
    body_content = fix_guillemet_bracket_pairs(body_content)
...
def process_volume(ie_id: str, ve_id: str, volume_id: str, volume_folder: Path, 
    ...
    try:
        ...
        # Output paths: archive and sources under {IE_ID-VE_ID}/{VOL_ID}
        folder_id = f"{ie_id}-{ve_id}"
        archive_dir = output_dir / "archive" / folder_id / (volume_id or "")
