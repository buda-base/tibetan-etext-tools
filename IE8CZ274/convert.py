...
# Supported file extensions for recursive discovery (RTF, DOC, and plain text)
...
def get_volume_rtf_files_from_toprocess(ve_ids: list, toprocess_path: Path = None):
    ...
    for ve_id in ve_ids:
        ...
        if not folder.is_dir():
            ...
            continue
        rtf_in_folder = (
            list(folder.rglob("*.rtf")) + list(folder.rglob("*.RTF"))
            + list(folder.rglob("*.txt")) + list(folder.rglob("*.TXT"))
        )
    ...
    logger.info(f"Found {total} total RTF/text files in toprocess ({len(ve_ids)} volume(s))")
    return result
...
def find_all_related_source_files(volume_base: str, rtf_dir: Path = None, doc_dir: Path = None) -> list:
    ...
    # Find TXT files (search recursively; for plain-text volumes)
    if rtf_dir.exists():
        for txt_file in rtf_dir.rglob(f"{volume_base}*.txt"):
...
def is_rtf_content(content: str) -> bool:
    """
    Return True if content looks like RTF (starts with {\\rtf).
    Used to treat plain-text files or misnamed files as text when they are not RTF.
    """
    if not content:
        return False
...
def _normalize_body_and_build_tei_xml(body_content: str, input_path: Path, doc_path: Path, ve_id: str, ut_id: str = None) -> str:
    """
    Apply normalization, <lb/> insertion, <hi> fixes, and wrap in TEI XML.
    Shared by RTF path (after building body from streams) and plain-text path.
    """
    ...
    body_content = re.sub(r'\s{2,}', ' ', body_content)
    ...
    # Add <lb/> at beginning of each new line
    ...
    source_file = doc_path if doc_path.exists() else input_path
...
<fileDesc>
<titleStmt>
<title>{escape_xml(input_path.stem)}</title>
</titleStmt>
<publicationStmt>
...
def convert_rtf_to_tei(rtf_path: Path, doc_path: Path, ve_id: str, ut_id: str = None,encoding="unicode") -> str:
    ...
    """
    # =========================================================================
    # STAGE 1: Parse RTF and Convert to Unicode (or treat as plain text)
    # =========================================================================
    logger.info(f"Parsing file: {rtf_path.name}")
    with open(rtf_path, encoding="utf-8", errors="ignore") as f:
        rtf_content = f.read()
    if not is_rtf_content(rtf_content):
        logger.info(f"Treating as plain text (not RTF): {rtf_path.name}")
        return _normalize_body_and_build_tei_xml(rtf_content, rtf_path, doc_path, ve_id, ut_id)
    ...
    return _normalize_body_and_build_tei_xml(body_content, rtf_path, doc_path, ve_id, ut_id)
...
def convert_all_files(output_dir: Path = None, encoding = "unicode"):
    ...
    if not ve_ids:
        ...
        if not RTF_DIR.exists():
            ...
            return
        all_rtf = find_files_recursive(RTF_DIR, {'.rtf', '.txt'})
        all_rtf = [f for f in all_rtf if '_output' not in f.parts]
        if not all_rtf:
            logger.error(f"No RTF or text files found under {RTF_DIR}")
            return
