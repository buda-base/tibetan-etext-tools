#!/usr/bin/env python3
"""
PDF to TEI XML Converter for IE3CN8142 (with Dedris font conversion)
...
Usage:
    ...
    python 2_convert_pdf_to_xml.py --single VE5CN1134/file.pdf  # Convert single file
    python 2_convert_pdf_to_xml.py --ve VE5CN1134               # Convert specific VE only
...
def strip_page_number_artifacts(text: str) -> str:
    ...
    """
    # Remove one or more consecutive lines that are only artifact chars,
    # when immediately before <lb/><pb/> or <pb/>
    pattern = (
        r'((?:<lb/>\s*[' + _PAGE_ARTIFACT_CHARS + r']*\n?)+)'
        r'(\s*<lb/>\s*<pb/>)'
    )
    text = re.sub(pattern, r'\2', text)
    # Also remove artifact line(s) directly before <pb/> (no <lb/> in between)
    pattern2 = (
        r'((?:<lb/>\s*[' + _PAGE_ARTIFACT_CHARS + r']*\n?)+)'
        r'(\s*<pb/>)'
    )
...
def convert_markup_to_tei(text: str) -> str:
    ...
    # Remove PDF footer artifacts (page numbers, tsheg runs, ུ) before each <pb/>
...
def get_all_pdf_files(ve_filter: str = None) -> dict:
...
def find_source_doc_file(pdf_path: Path, ve_id: str) -> Path:
    ...
    for doc_path in ve_folder.glob(f"*.doc"):
...
def convert_all_files(ve_filter: str = None):
    ...
    pdf_by_ve = get_all_pdf_files(ve_filter)
