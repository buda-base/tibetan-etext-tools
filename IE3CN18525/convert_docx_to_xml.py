#!/usr/bin/env python3
"""
DOCX to TEI XML Converter for IE3CN18525

This script converts DOCX files to TEI XML format using the BasicDOCX parser.
IE3CN18525 DOCX files are already in Unicode, so no Dedris conversion is needed.

IE3CN18525 reads DOCX files from two sources:
1. Intermediate DOCX files in docx/{VE_ID}/ (converted from DOC)
2. Original DOCX files in toprocess/{IE_ID}-{VE_ID}/

Usage:
    python convert_docx_to_xml.py                          # Convert all DOCX files
    python convert_docx_to_xml.py --single VE5CN976/file.docx  # Convert single file from docx/
    python convert_docx_to_xml.py --single VE5CN976/file.docx --sequence 1  # With specific sequence
"""
...
def convert_docx_to_tei(docx_path: Path, ve_id: str, sequence: int, source_type: str) -> str:
    ...
    IE3CN18525 DOCX files are already Unicode - no Dedris conversion needed.
