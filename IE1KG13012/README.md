...
### 2026-02-16: IE1KG13012 Specific Fixes

#### Font Table Parsing Fix (`basic_rtf.py`)
- **Issue**: Font names were not being extracted from "simple" RTF format
  - RTF files with font entries NOT individually wrapped in braces were failing
  - Example: `{\fonttbl\f0...;\f1...;}` instead of `{\fonttbl{\f0...}{\f1...}}`
  - This caused empty font names, preventing Dedris-to-Unicode conversion
  - Result: Garbled ASCII characters like `3*3`, `!R/`, `$*A?` instead of Tibetan text

- **Solution**: Updated `_parse_font_table_simple()` to handle both formats:
  1. First tries to parse entries with individual braces (original logic)
  2. If no braced entries found, parses as semicolon-separated list by splitting on `\f` control sequences
  
- **Impact**: Now correctly extracts all font names (Dedris-vowa, Dedris-a, Dedris-b, Dedris-a1, Dedris-d)
  - Enables proper Dedris-to-Unicode conversion
  - Produces clean Tibetan Unicode text instead of garbled ASCII

#### Pattern Removal (`tibetan_text_fixes.py`, `convert.py`)
- **Issue**: Unwanted pattern `་ ་ ་ ་ ་` (5 repeated tseg with spaces) appeared at start of files

- **Solution**: 
  - Added `remove_repeated_tseg_pattern()` function in `tibetan_text_fixes.py`
  - Integrated into conversion pipeline in `convert.py` after Unicode normalization
  - Removes both `་ ་ ་ ་ ་ ` (with trailing space) and `་ ་ ་ ་ ་` (without)

- **Impact**: Clean text output without unwanted repeated tseg patterns
