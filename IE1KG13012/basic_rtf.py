...
def _parse_font_table_simple(fonttbl_data: str) -> list:
    ...
    Simple Dedris fonts can appear in two formats:
    1. Without individual braces: \f3\fnil\fcharset0 Dedris-a;
    2. With individual braces: {\f3\fnil\fcharset0 Dedris-a;}
    
    System fonts may have panose: {\f0\froman...\fprq2{\*\panose...}Times New Roman;}
    """
    fonts = []
    
    # First try to find entries with individual braces
    brace_level = 0
    entry_start = -1
    found_braced_entries = False
    ...
    # If no braced entries found, try parsing as semicolon-separated list
    if not found_braced_entries:
        # Split by \f to get each font entry
        entries = re.split(r'\\f(\d+)', fonttbl_data)
        
        # Process pairs: (font_id, font_data)
        for i in range(1, len(entries), 2):
            if i + 1 >= len(entries):
                break
            
            fid = int(entries[i])
            font_data = entries[i + 1]
            
            # Get text up to semicolon
            if ';' in font_data:
                font_data = font_data.split(';')[0]
            
            # Remove control words
            cleaned = re.sub(r'\\[a-zA-Z]+\-?\d*\s?', '', font_data)
