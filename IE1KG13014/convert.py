...
def dedris_to_unicode(text: str, font_name: str) -> str:
    ...
    # SPECIAL CASE: If text contains only ASCII dots or spaces,
    # don't convert (these are likely table of contents dots)
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    # Replace Tibetan tsheg (་) with ASCII hyphen (-) when it appears between Tibetan digits
    # This fixes dates like ༡༨༠༨་༡༨༨༧ → ༡༨༠༨-༡༨༨༧
    # Tibetan digits are U+0F20-U+0F29
    body_content = re.sub(r'([༠-༩])་([༠-༩])', r'\1-\2', body_content)
    
    # Remove ªê comes from [2 symbol in source
    body_content = body_content.replace('ªê', '')
    #body_content = body_content.replace('ªë', '')  # Similar pattern
