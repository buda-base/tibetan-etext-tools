#!/usr/bin/env python3
"""
Step 2 (RTF Pipeline): RTF to TEI XML Converter
...
Usage:
    ...
    python 2_convert_rtf_to_xml.py --single VE5CN1134/file.rtf  # Convert single file
    python 2_convert_rtf_to_xml.py --ve VE5CN1134  # Convert all files for specific VE ID
"""
...
def get_all_rtf_files(ve_filter: str = None) -> dict:
    """
    ...
    Returns:
        Dict mapping VE ID to list of RTF file paths
    """
    rtf_by_ve = {}
...
def find_source_doc(ve_id: str, rtf_stem: str) -> Path:
    """
    Find the source DOC file for an RTF file.
    
    Args:
        ve_id: The VE ID (e.g., 'VE5CN1134')
        rtf_stem: The RTF filename without extension
        
    Returns:
        Path to the source DOC file, or None if not found
    """
    folder_name = f"{IE_ID}-{ve_id}"
    doc_path = TOPROCESS_DIR / folder_name / f"{rtf_stem}.doc"
    
    if doc_path.exists():
        return doc_path
    
    for doc_file in (TOPROCESS_DIR / folder_name).glob("*.doc"):
        if doc_file.stem.lower() == rtf_stem.lower():
            return doc_file
...
def convert_rtf_to_tei(rtf_path: Path, ve_id: str, sequence: int) -> str:
    ...
    doc_path = find_source_doc(ve_id, rtf_path.stem)
    ...
    sha256 = calculate_sha256(doc_path) if doc_path else ""
    src_path = f"sources/{ve_id}/{doc_path.name}" if doc_path else f"sources/{ve_id}/{rtf_path.stem}.doc"
...
def convert_all_files(ve_filter: str = None):
    ...
    rtf_by_ve = get_all_rtf_files(ve_filter)
