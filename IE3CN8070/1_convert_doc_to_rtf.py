#!/usr/bin/env python3
"""
Step 1 (RTF Pipeline): DOC to RTF Converter using Microsoft Word COM Automation

This script converts DOC files to RTF format using Microsoft Word.
It processes DOC files in the toprocess/{IE_ID}-{VE_ID}/ folders and outputs
RTF files to rtf/{VE_ID}/ preserving the folder structure.

Usage:
    python 1_convert_doc_to_rtf.py           # Convert all DOC files
    python 1_convert_doc_to_rtf.py --single IE3CN8070-VE5CN1134/file.doc  # Convert single file
    python 1_convert_doc_to_rtf.py --ve VE5CN1134  # Convert all files for specific VE ID
"""
...
def get_all_doc_files(ve_filter: str = None) -> list:
    """
    ...
    Returns:
        List of (doc_path, ve_id) tuples
    """
    doc_files = []
    
    if not TOPROCESS_DIR.exists():
        logger.error(f"Source directory not found: {TOPROCESS_DIR}")
        return []
...
def convert_all(ve_filter: str = None):
    """Convert all DOC files to RTF."""
    logger.info("=" * 70)
    logger.info(f"DOC to RTF Converter for {IE_ID}")
    if ve_filter:
    ...
    doc_files = get_all_doc_files(ve_filter)
