...
Examples:
    ...
    # Skip glyph-shape fallback (faster, less accurate for bad PDFs)
    python pdf_to_tei_xml.py mybook.pdf ./output --no-shape-fallback
"""
...
def decode_glyphs(pages_glyphs: list[list[GlyphInfo]],
    ...
    Priority:
      ...
      3. Optionally: shape-based fallback (placeholder – extend as needed).
    """
...
def pages_to_tagged_text(pages_glyphs: list[list[GlyphInfo]]) -> str:
    """
    ...
    for page in pages_glyphs:
        ...
        line_texts = [_line_to_tagged_text(l) for l in lines]
        line_texts = [t for t in line_texts if t.strip()]
        page_texts.append("\n".join(line_texts))
...
def fix_tibetan_mark_order(text: str) -> str:
    """
    Fix common OCR/PDF ordering issues where tsheg (་) appears before Tibetan
    combining marks (for example: འ་ི -> འི་).
    """
    tsheg = "\u0F0B"
    chars = list(text)
    i = 0
...
def convert_markup_to_tei(text: str) -> str:
    ...
    text = re.sub(r"(<pb/>[\s\n]*)+", r"<pb/>\n", text)
    text = _normalize_paginated_tei_lines(text)

    return text


def _normalize_paginated_tei_lines(text: str) -> str:
    """
    Enforce paginated TEI line-shape constraints from the spec:
      - no empty file lines
      - <pb/> (optionally with attributes) is alone on its own line
      - content lines start with a single <lb/>
      - no consecutive empty line-break markers (<lb/> with no content)
    """
    # Keep page-break tags on isolated file lines.
    text = re.sub(r"\s*(<pb(?:\s+[^/>]+)?/>)\s*", r"\n\1\n", text)
    text = re.sub(r"\n{2,}", "\n", text).strip()

    lines: list[str] = []
    lb_empty_pat = re.compile(r"^<lb/>\s*$")
    pb_pat = re.compile(r"^<pb(?:\s+[^/>]+)?/>\s*$")
    ...
    for raw in text.splitlines():
        ...
        if pb_pat.match(line):
            lines.append(line)
            continue

        # Remove duplicated leading <lb/> tags, then enforce one <lb/> prefix.
        content = re.sub(r"^(?:<lb/>\s*)+", "", line).strip()
        if not content:
            continue
        lines.append(f"<lb/>{content}")

    # Remove duplicate adjacent content line markers.
    normalized: list[str] = []
    for line in lines:
        if lb_empty_pat.match(line):
            continue
        if normalized and normalized[-1] == "<lb/>" and line.startswith("<lb/>"):
            continue
...
def convert_pdf(pdf_path: Path,
                ...
                file_idx: int       = 1,
                crop: Optional[tuple] = None,
                use_shape_fallback: bool = False) -> Optional[Path]:
    """
    ...
    raw_text = pages_to_tagged_text(pages_glyphs)
...
def main():
    ...
    parser.add_argument("--no-shape-fallback", action="store_true",
        help="Disable slow shape-based glyph matching (Phase 2 step 3)")
    ...
    use_shape = not args.no_shape_fallback
