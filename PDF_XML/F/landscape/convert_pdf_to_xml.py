#!/usr/bin/env python3
"""
PDF to TEI XML converter for landscape-oriented Tibetan PDFs.
Extracts text using pytiblegenc with optional PyMuPDF-based header/footer
redaction, applies Dedris conversion, Unicode normalisation, font-size
classification, and generates TEI XML output.
...
Usage:
    python convert_pdf_to_xml.py                                          # Convert all PDFs
    python convert_pdf_to_xml.py --ve VE1ER1060                           # One VE only
    ...
    python convert_pdf_to_xml.py --assign-flat-toprocess                  # assign root *.pdf via toprocess
    python convert_pdf_to_xml.py --no-font-tags
    python convert_pdf_to_xml.py --no-normalization
    python convert_pdf_to_xml.py --crop-top 0.09 --crop-bottom 0.08      # redact header/footer bands
"""
...
_t1_dir = script_dir.parent
# Parent path: pdf_extract. This script_dir last on path so `import config` is
# landscape/config.py, not T1/config.py.
if str(_t1_dir) not in sys.path:
    sys.path.insert(0, str(_t1_dir))
...
from tibetan_text_fixes import (
    ...
    replace_odd_footer_page_lines_with_pb,
    strip_standalone_dkar_chag_kha_lines,
)
...
try:
    from pytiblegenc import pdf_to_txt  # noqa: F401 — ensures pytiblegenc is installed
    from pdf_extract import extract_pdf_to_text_full
...
# CLI overrides for pdf_extract (merged with config in extract_pdf_to_text).
_EXTRACT_RUNTIME: dict = {}
...
def extract_pdf_to_text(pdf_path: Path) -> str:
    """Extract text from a PDF file using pytiblegenc (with optional two-column / boxes_flow)."""
    logger.info(f"    Extracting: {pdf_path.name}")

    try:
        two_column = _EXTRACT_RUNTIME.get("two_column", PDF_TWO_COLUMN_LANDSCAPE)
        column_order = _EXTRACT_RUNTIME.get("column_order", PDF_COLUMN_ORDER)
        column_split = _EXTRACT_RUNTIME.get("column_split", PDF_COLUMN_SPLIT)
        boxes_flow = _EXTRACT_RUNTIME.get("boxes_flow", PDF_BOXES_FLOW)
        extract_region = _EXTRACT_RUNTIME.get("extract_region", PDF_EXTRACT_REGION)
        crop_top = float(_EXTRACT_RUNTIME.get("crop_top", CROP_HEADER_FRACTION))
        crop_bottom = float(_EXTRACT_RUNTIME.get("crop_bottom", CROP_FOOTER_FRACTION))
        if two_column:
            logger.info(
                f"    Layout: two-column order={column_order} split={column_split} boxes_flow={boxes_flow}"
            )
        elif boxes_flow is not None:
            logger.info(f"    Layout: boxes_flow={boxes_flow}")
        if crop_top > 0 or crop_bottom > 0:
            logger.info(
                f"    Crop (extraction region): top={crop_top*100:.1f}% bottom={crop_bottom*100:.1f}% "
                f"of page height "
                f"({'per column at split ' + str(column_split) if two_column else 'full width'})"
            )
        text = extract_pdf_to_text_full(
            str(pdf_path),
            page_break_str=f"\n{PAGE_BREAK_STR}\n",
...
def convert_markup_to_tei(text: str) -> str:
    ...
    # Font span opened before a page break: close hi before pb
    text = re.sub(
        r'(<hi rend="(?:head|small)">)((?:(?!(?:</hi>|<pb/>)).)*)\s*<pb/>',
        r"\1\2</hi><pb/>",
        text,
        flags=re.DOTALL,
    )
...
def convert_pdf_to_tei(pdf_path: Path, ve_id: str, sequence: int) -> str:
    ...
    tei_body = fix_small_wrapped_volume_running_title(tei_body)
    tei_body = strip_standalone_dkar_chag_kha_lines(tei_body)
    tei_body = fix_split_hi_close_digit_footer_line(tei_body)
    tei_body = fix_hi_balance(tei_body)
    tei_body = replace_odd_footer_page_lines_with_pb(tei_body)
...
def main():
    ...
    parser.add_argument(
        "--two-column",
        action="store_true",
        help="Force two-column per-page extract (see config PDF_TWO_COLUMN_LANDSCAPE)",
    )
    parser.add_argument(
        "--no-two-column",
        action="store_true",
        help="Disable two-column extract even if enabled in config",
    )
    parser.add_argument(
        "--column-order",
        choices=("lr", "rl"),
        default=None,
        help="Two-column band order: left-right or right-left (overrides config)",
    )
    parser.add_argument(
        "--column-split",
        type=float,
        default=None,
        metavar="X",
        help="Relative x (0-1) where right column starts (overrides config)",
    )
    parser.add_argument(
        "--boxes-flow",
        type=float,
        default=None,
        metavar="F",
        help="miner boxes_flow (overrides config PDF_BOXES_FLOW)",
    )
    parser.add_argument(
        "--crop-top",
        type=float,
        default=None,
        metavar="F",
        help="Exclude top F of page height from extraction (0–0.49; pdfminer region, no PDF rewrite).",
    )
    parser.add_argument(
        "--crop-bottom",
        type=float,
        default=None,
        metavar="F",
        help="Exclude bottom F of page height from extraction (0–0.49). Overrides config CROP_FOOTER_FRACTION.",
    )
    parser.add_argument(
    ...
    global ENABLE_FONT_CLASSIFICATION, ENABLE_NORMALIZATION, _EXTRACT_RUNTIME
    ...
    if args.no_two_column:
        _EXTRACT_RUNTIME["two_column"] = False
    elif args.two_column:
        _EXTRACT_RUNTIME["two_column"] = True
    if args.column_order is not None:
        _EXTRACT_RUNTIME["column_order"] = args.column_order
    if args.column_split is not None:
        _EXTRACT_RUNTIME["column_split"] = args.column_split
    if args.boxes_flow is not None:
        _EXTRACT_RUNTIME["boxes_flow"] = args.boxes_flow
    if args.crop_top is not None:
        if not 0.0 <= args.crop_top < 0.5:
            parser.error("--crop-top must be >= 0 and < 0.5")
        _EXTRACT_RUNTIME["crop_top"] = args.crop_top
    if args.crop_bottom is not None:
        if not 0.0 <= args.crop_bottom < 0.5:
            parser.error("--crop-bottom must be >= 0 and < 0.5")
        _EXTRACT_RUNTIME["crop_bottom"] = args.crop_bottom
