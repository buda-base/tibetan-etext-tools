...
# -------------------------------------------------------------------------
# Collapse repeated Tibetan combining marks (vowels, subscripts, marks).
# U+0F71..U+0F84 covers all combining vowels and marks; U+0F90..U+0FBC
# covers all subjoined consonants.  Repetition of the same combining
# character is never valid Tibetan and is a PDF extraction artefact.

_REPEATED_COMBINING_MARK_RE = re.compile(
    r"([\u0F71-\u0F84\u0F90-\u0FBC])\1+"
)


def collapse_duplicate_marks(text: str) -> str:
    """Collapse runs of the same Tibetan combining mark to a single instance."""
    if not text:
        return text
    return _REPEATED_COMBINING_MARK_RE.sub(r"\1", text)
...
def normalize_unicode(
    ...
    # 0) Strip spurious subjoined consonants from Qomolangma font ligatures.
    #    Must run BEFORE NFC so the two-codepoint sequences are still intact.
    from tibetan_text_fixes import fix_qomolangma_ligatures
    s = fix_qomolangma_ligatures(text)
    ...
    # 8) Collapse duplicate combining marks (PDF extraction artefact)
    s = collapse_duplicate_marks(s)
