#!/usr/bin/env python3
"""
IE2KG234649: PDF in sources/ to TEI XML (Dedris via pytiblegenc)
...
volumes run; a mismatched folder like toprocess/IE2KG234649-VE1ER9999 does not affect VE1ER1021.

Optional: flat sources/*.pdf — use ``--assign-flat-toprocess`` to split those across
IE2KG234649-VE* folders in toprocess; otherwise root PDFs are skipped (move into sources/<VE_ID>/).

toprocess is still checked only for optional matching .doc (IE2KG234649-<same VE as source>) for SHA256.
Output: IE2KG234649_output/archive/{VE_ID}/UT*.xml and sources/{VE_ID}/ (PDF + optional DOC).

Optional: matching .doc in toprocess/IE2KG234649-{ve_id}/ for SHA256; otherwise SHA256 from PDF.

Actual paths and ``IE_ID`` come from ``config.py``; adjust that file for your machine.
