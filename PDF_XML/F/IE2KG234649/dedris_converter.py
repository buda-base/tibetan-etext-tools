...
def dedris_to_unicode(text: str, font_name: str) -> str:
    ...
    Args:
        ...
        font_name: Font name from RTF/DOCX (e.g., "Dedris-a", "Dedris-vowa")
        
    Returns:
        Unicode text (unchanged if font is not Dedris)
    """
    ...
    # Check for mixed content (Tibetan Unicode + ASCII Dedris)
    has_tibetan = any(0x0F00 <= ord(c) <= 0x0FFF for c in text)
    dedris_ascii_chars = set('.{}0\\/(),-')
    has_dedris_ascii = any(c in dedris_ascii_chars for c in text)
    is_mixed = has_tibetan and has_dedris_ascii
    ...
    if not is_dedris:
        # Skip conversion entirely for non-Dedris fonts
        # Don't try to guess based on text patterns - only convert if font is explicitly Dedris
        return text
    
    # Font is Dedris - convert using pytiblegenc
    try:
