#!/usr/bin/env python3
"""
PDF to TEI XML (pytiblegenc) with embedded-font CID patching and optional redaction.

Extraction uses pytiblegenc (pdfminer).  ``glyph_decoder.patch_pytiblegenc_cid_decoder`` patches
the PDF text converter so CIDs map correctly for this corpus (see ``glyph_decoder.py`` /
``CID_MAPPING_README.md``).

Optional header/footer removal uses PyMuPDF redaction annotations to physically blank page bands
before ``pdf_to_txt``, so text in those regions is not extracted (unlike cropbox-only cropping).

Input:  PDFs under ``sources/<VE_ID>/`` (volume ID = folder name; see ``config.py`` for ``BASE_DIR``).
Output: ``{IE_ID}_output/archive/<VE_ID>/UT*.xml`` and ``sources/<VE_ID>/`` copies (``IE_ID`` from config).

Optional: matching ``.doc`` in ``toprocess/{IE_ID}-<VE_ID>/`` for SHA256; otherwise SHA256 from the PDF.

Usage:
    python test.py                                              # Convert all PDFs
    python test.py --ve VE1ER1060                               # One VE only
    python test.py --single foo.pdf
    python test.py --single VE1ER1060/TI1188-01-001.pdf        # path under sources/
...
from dedris_converter import reset_stats, print_conversion_stats, write_stats_file, convert_string, STATS
...
def force_legacy_conversion(text: str) -> str:
    """Forces ASCII legacy text to Tibetan Unicode while protecting XML tags."""
    parts = re.split(r'(<[^>]+>)', text)
    converted_parts = []
    
    for part in parts:
        if part.startswith('<') and part.endswith('>'):
            converted_parts.append(part) # Keep tags intact
        elif part.strip():
            # Force pytiblegenc to treat this chunk as 'dedris'
            converted = convert_string(part, "dedris", STATS)
            converted_parts.append(converted)
        else:
            converted_parts.append(part) # Keep whitespace intact
...
def get_ve_ids_from_toprocess():
    """Collect VE IDs from toprocess folders named {IE_ID}-VE* (IE_ID from config)."""
    ve_ids = []
...
def convert_pdf_to_tei(pdf_path: Path, ve_id: str, sequence: int) -> str:
    ...
    # FORCE LEGACY CONVERSION: Fix raw ASCII (like "h;¼-VG") turning it into Tibetan Unicode
    converted_raw_text = force_legacy_conversion(raw_text)

    # Use the converted text down the rest of the pipeline
    simplified_text = simplify_font_sizes(converted_raw_text)

    if ENABLE_NORMALIZATION:
        logger.info("    Applying normalization...")
        normalized_text = normalize_unicode(simplified_text)
        
        # Repair flying vowels/subscripts that break across line-breaks during extraction
        normalized_text = fix_flying_vowels_and_linebreaks(normalized_text)
    else:
...
def _resolve_ve_for_single(explicit_ve: Optional[str]) -> Optional[str]:
    """
    For a PDF directly in sources/*.pdf (no subfolder): use --ve, or one IE2KG234648-VE* in toprocess.
...
def convert_all_files(
    ...
    if not pdf_by_ve or total_files == 0:
        logger.error(
            ...
            "(e.g. sources/VE1ER1060/) or use --assign-flat-toprocess with IE2KG234648-VE* in toprocess."
        )
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="IE2KG234648: Convert sources/*.pdf and sources/<VE>/**/*.pdf to TEI XML (pytiblegenc + CID patch)"
    )
    parser.add_argument(
        "--single",
        "-s",
    ...
    parser.add_argument("--ve", metavar="VE_ID", help="VE ID (e.g. VE1ER1060) or filter batch to this VE")
    parser.add_argument(
        "--sequence",
        type=int,
        default=None,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Also assign sources/*.pdf (root) across IE2KG234648-VE* folders in toprocess (default: ignore root PDFs)",
    )
    parser.add_argument(
        "--crop-top",
        type=float,
