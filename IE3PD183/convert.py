...
def convert_rtf_to_tei(rtf_path: Path, doc_path: Path, ve_id: str, ut_id: str = None,encoding="unicode") -> str:
    ...
    # Read as latin-1 so Dedris 8-bit bytes (0x80-0xFF) are preserved; UTF-8 would drop them (errors="ignore")
    with open(rtf_path, encoding="latin-1") as f:
        rtf_content = f.read()
    rtf_content = strip_rtf_picture_groups(rtf_content)
    tmp_fd, tmp_path = tempfile.mkstemp(suffix=".rtf", prefix="ie1kg4884_")
    try:
        # Write temp file as latin-1 so parser sees same byte stream (Dedris 0x80-0xFF preserved)
        with os.fdopen(tmp_fd, "w", encoding="latin-1") as tf:
...
def convert_all_files(output_dir: Path = None, encoding = "unicode",srctype = "rtf"):
    ...
    if not ve_ids:
        ...
        all_files = [f for f in all_files if not any("_output" in p for p in f.parts)]
        if not all_files:
