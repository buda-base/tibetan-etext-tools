...
# Tibetan bracket characters (often rendered small): ༼ U+0F3C, ༽ U+0F3D
TIBETAN_OPEN_BRACKET = '\u0F3C'   # ༼
TIBETAN_CLOSE_BRACKET = '\u0F3D'  # ༽

# Match <hi rend="small"> or <hi rend='small'>
HI_SMALL_OPEN_RE = re.compile(r'<hi\s+rend=["\']small["\']\s*>', re.IGNORECASE)


def _find_matching_hi_close(text: str, start: int) -> Tuple[int, int]:
    """Return (inner_end, outer_end): inner_end = index of start of </hi>, outer_end = index past </hi>.
    start is the position just after the opening <hi ...> tag. (-1, -1) if not found."""
    depth = 1  # we are already inside one <hi>
    i = start
    while i < len(text):
        if i + 5 <= len(text) and text[i] == '<':
            if text[i:i + 5] == '</hi>':
                depth -= 1
                if depth == 0:
                    return (i, i + 5)  # start of </hi>, past </hi>
                i += 5
                continue
            if i + 4 <= len(text) and text[i:i + 3] == '<hi' and text[i + 3:i + 4] in (' ', '>'):
                depth += 1
                gt = text.find('>', i + 3)
                if gt == -1:
                    return (-1, -1)
...
def flatten_hi_small_brackets(text: str) -> Tuple[str, int]:
    """
    Replace nested or wrapped <hi rend="small"> around ༼...༽ with a flat structure:
    <hi rend="small">༼</hi>middle<hi rend="small">༽</hi>
    so the brackets are each in their own hi tag and the middle is unwrapped.
    """
    fix_count = 0
    result = []
    i = 0
    while i < len(text):
        m = HI_SMALL_OPEN_RE.search(text, i)
        if not m:
        ...
        open_start = m.start()
        open_end = m.end()
        inner_end, outer_end = _find_matching_hi_close(text, open_end)
        if inner_end == -1:
            result.append(text[open_start:])
            break
        inner = text[open_end:inner_end]
        if TIBETAN_OPEN_BRACKET in inner and TIBETAN_CLOSE_BRACKET in inner:
            idx_open = inner.index(TIBETAN_OPEN_BRACKET)
            idx_close = inner.rindex(TIBETAN_CLOSE_BRACKET)
            if idx_close > idx_open:
                before = inner[:idx_open]
                middle = inner[idx_open + 1:idx_close]
                after = inner[idx_close + 1:]
                middle = re.sub(r'<hi\s+rend=["\']small["\']\s*>', '', middle, flags=re.I)
                middle = re.sub(r'</hi>', '', middle)
                replacement = (
                    before
                    + '<hi rend="small">' + TIBETAN_OPEN_BRACKET + '</hi>'
                    + middle
                    + '<hi rend="small">' + TIBETAN_CLOSE_BRACKET + '</hi>'
                    + after
                )
                result.append(replacement)
                fix_count += 1
            else:
                result.append(text[open_start:outer_end])
        else:
            result.append(text[open_start:outer_end])
        i = outer_end
...
def fix_xml_file(
    ...
    result = {
        ...
        'tsheg_run_spaces_removed': 0,
        'hi_small_brackets_flattened': 0,
        'total_fixes': 0,
        'error': None
    }
    ...
    try:
        ...
        # Remove space/tab after ་ and ། when next char is Tibetan, only if body has "many space" artifact
        tsheg_count = 0
        if body_has_many_tsheg_spaces(cleaned_body):
            cleaned_body, tsheg_count = clean_tsheg_run_spaces(cleaned_body)
            result['tsheg_run_spaces_removed'] = tsheg_count
        
        # Flatten nested <hi rend="small"> around ༼...༽ to <hi rend="small">༼</hi>middle<hi rend="small">༽</hi>
        cleaned_body, hi_bracket_count = flatten_hi_small_brackets(cleaned_body)
        result['hi_small_brackets_flattened'] = hi_bracket_count
        
        result['total_fixes'] = (
            rtf_count + spurious_count + hi_wrappers_count + corruption_count
            + result['source_correction_fixes'] + non_tibetan_count + tsheg_count + hi_bracket_count
        )
...
def scan_xml_file(xml_path: Path) -> Dict:
    ...
    try:
        ...
        # Also treat Dedris corruption, short heads, spurious (full-body), and empty <hi> tags as issues.
        # Full-body spurious check ensures we mark files for fix when match is beyond line cap (8000 chars).
        ...
        has_spurious = body_has_spurious(body_text)
        has_many_tsheg_spaces = body_has_many_tsheg_spaces(body_text)
        has_empty_hi = bool(EMPTY_HI_RE.search(body_text))
        result['has_issues'] = (
            len(rtf_issues) > 0 or len(non_tibetan) > 0
            or has_corruption or has_short_head or has_spurious or has_empty_hi or 
            has_many_tsheg_spaces
        )
...
def _available_collection_ids(input_dir: Path) -> list:
    """Return list of collection IDs that have {id}/{id}_output/archive under input_dir."""
    ids = []
    try:
        for d in input_dir.iterdir():
            if d.is_dir() and (d / f"{d.name}_output").exists():
                archive = d / f"{d.name}_output" / "archive"
                if archive.exists():
                    ids.append(d.name)
    except OSError:
        pass
...
def _resolve_ie_id(ie_id_filter: str, input_dir: Path) -> str:
    """
    Return the collection ID to use. If exact match exists, return it.
    If no exact match, try to find a single collection that contains the filter
    (e.g. IE3CN918 -> IE3CN4918) or that the filter is a prefix of; otherwise return None.
    """
    available = _available_collection_ids(input_dir)
    if ie_id_filter in available:
        return ie_id_filter
    # No exact match: try substring/prefix so typos like IE3CN918 find IE3CN4918
    containing = [aid for aid in available if ie_id_filter in aid or aid.startswith(ie_id_filter)]
    if len(containing) == 1:
        return containing[0]
    # Filter is almost the same (e.g. IE3CN918 -> IE3CN4918): same short prefix and filter's tail digits in candidate
    prefix = ie_id_filter[:5] if len(ie_id_filter) >= 5 else ie_id_filter
    tail = ie_id_filter[5:] if len(ie_id_filter) > 5 else ""  # e.g. "918" from IE3CN918
    similar = [
        aid for aid in available
        if aid.startswith(prefix)
        and abs(len(aid) - len(ie_id_filter)) <= 2
        and (not tail or tail in aid[5:])  # e.g. 918 in 4918
    ]
    if len(similar) == 1:
        return similar[0]
    return None
...
def scan_collection(input_dir: Path, verbose: bool = False, ie_id_filter: str = None, workers: int = DEFAULT_WORKERS) -> Dict:
    ...
    if ie_id_filter:
        ...
        if not filtered_files:
            resolved_id = _resolve_ie_id(ie_id_filter, input_dir)
            if resolved_id and resolved_id != ie_id_filter:
                for xml_file in xml_files:
                    parts = xml_file.parts
                    for i, part in enumerate(parts):
                        if part == resolved_id and i + 1 < len(parts) and parts[i + 1] == f"{resolved_id}_output":
                            filtered_files.append(xml_file)
                            break
                if filtered_files:
                    print(f"Note: No exact match for --ie-id {ie_id_filter}; using collection {resolved_id}")
        if resolved_id:
            if not filtered_files:
                xml_files = []
            else:
                xml_files = filtered_files
                ie_id_filter = resolved_id  # use resolved for rest of function
        if not xml_files and ie_id_filter:
            available = _available_collection_ids(input_dir)
            print(f"Error: No XML files found for collection {ie_id_filter}")
            if available:
                print(f"  Available collections: {', '.join(available)}")
            else:
                print(f"  Expected path: {input_dir}/<ie_id>/<ie_id>_output/archive/")
            return results
...
def fix_collection(
    ...
    results = {
        ...
        'total_tsheg_run_spaces_removed': 0,
        'total_hi_small_brackets_flattened': 0,
        'total_fixes': 0,
        'errors': []
    }
            ...
            try:
                for future in as_completed(pending, timeout=AS_COMPLETED_TIMEOUT):
                    ...
                    if fix_result.get('error'):
                        ...
                        results['total_tsheg_run_spaces_removed'] += fix_result.get('tsheg_run_spaces_removed', 0)
                        results['total_hi_small_brackets_flattened'] += fix_result.get('hi_small_brackets_flattened', 0)
...
def main():
    ...
    if not args.no_fix:
        ...
        if response == 'y':
            ...
            print(f"Tsheg run spaces removed: {fix_results['total_tsheg_run_spaces_removed']}")
            print(f"Hi small brackets flattened: {fix_results['total_hi_small_brackets_flattened']}")
