...
All detection logic and shared constants live here. Used by rtf_check_fix to
decide which files have issues, and by rtf_cleaner (imports patterns/constants)
to perform cleaning. No cleaning is done in this module.
"""
...
SPURIOUS_PATTERNS = [
    ...
    (r'\u00BF', 'Spurious inverted question mark'),
...
# --- Detection: HI (head) wrappers and empty HI ---
...
# Tibetan punctuation used in detection (and by cleaner for tsheg-run-space fix)
TIBETAN_TSHEG = '\u0F0B'   # ་ — syllable boundary
SHAD = '\u0F0D'            # ། — phrase/sentence boundary
# Empty <hi rend="..."></hi> (no content or only whitespace) — detected for fix, cleaned in rtf_cleaner
...
MAX_BODY_LEN_FOR_SPURIOUS_SCAN = 400_000


# --- Detection functions (used by rtf_check_fix for scan and fix decisions) ---


def body_has_spurious(text: str) -> bool:
    """Return True if any spurious pattern matches anywhere in body (capped length). Use so files get fixed even when the match is beyond MAX_LINE_LEN_FOR_REGEX on a single line."""
    capped = text[:MAX_BODY_LEN_FOR_SPURIOUS_SCAN]
    return any(pattern_re.search(capped) for pattern_re, _ in SPURIOUS_PATTERNS_COMPILED)
...
def body_has_many_tsheg_spaces(
    text: str,
    threshold: float = 0.6,
    min_tsheg: int = 10,
) -> bool:
    """
    Detect if the body has the RTF run-boundary artifact: ་ (tsheg) is very often
    followed by a space before the next syllable (e.g. རིག་ འཛིན་ གཏེར་).
    Only files with this pattern should get tsheg run-space cleaning applied.
    """
    total_tsheg = text.count(TIBETAN_TSHEG)
    if total_tsheg < min_tsheg:
        return False
    tsheg_followed_by_space = text.count(TIBETAN_TSHEG + ' ')
    ratio = tsheg_followed_by_space / total_tsheg
