...
def detect_rtf_format(file_path: str, encoding: str = "latin-1") -> str:
    """
    Detect RTF format by checking if Dedris fonts have {\*\panose}.
    
    Args:
        file_path: Path to RTF file
        encoding: File encoding (latin-1 preserves Dedris 8-bit bytes; default for RTF from convert.py)
...
class BasicRTF:
    ...
    def parse_file(self, file_path, show_progress=True):
        ...
        # Detect format (use latin-1 so Dedris 8-bit bytes are preserved when temp file is written as latin-1 by convert.py)
        self._format = detect_rtf_format(file_path, encoding="latin-1")
        if show_progress:
        ...
        stack = []
        caption_collect = False  # True after \shptxt or \dptxbxtext inside a {\*} caption group
        ...
        # \header/\footer removed: some RTFs put body text inside {\header}; parse as body
        special_keywords = {
            r'\footnote': 'footnote',
            r'\pict': 'pict'
        }

        # Skip these sections entirely (do not skip {\header/{\footer: some RTFs put body text there)
        skip_sections = [
        ...
        while i < data_len:
            ...
            c = data[i]
            # Skip text until we see \shptxt/\dptxbxtext when inside a {\*} caption group (anywhere on stack)
            in_caption_skip = bool(any(len(tos) >= 6 and tos[5] for tos in stack) and not caption_collect)
            ...
            if c == '\\':
                for kw, typ in special_keywords.items():
                    if data.startswith(kw, i):
                        ...
                        if typ == 'pict' and stack:
                            # Skip the entire group that contains \pict (image data) so that
                            # any following text (e.g. \shptxt description/caption) is parsed.
                            group_start = stack[-1][4]
                            brace_level = 1
                            k = group_start + 1
                            while k < data_len and brace_level > 0:
                                if data[k] == '{':
                                    ...
                                    brace_level -= 1
                                k += 1
                            block_end = k - 1  # position of closing }
                            stack.pop()
                            i = block_end + 1
                            special_type = typ
                            break
                        # Find the opening brace for this block (header, footer, footnote)
                        j = end_pos
            ...
            # Handle groups starting with \* (ignore group), unless they contain caption text
            if c == '{' and i + 2 < data_len and data[i+1] == '\\' and data[i+2] == '*':
                ...
                while j < data_len and brace_level > 0:
                    if data[j] == '{':
                        ...
                        brace_level -= 1
                    j += 1
                group_content = data[i + 1 : j - 1]
                # Don't skip if group contains shape/drawing text (caption/description)
                if '\\shptxt' in group_content or '\\dptxbxtext' in group_content:
                    # Enter the group so we parse \shptxt / \dptxbxtext caption content (6th=True = caption group)
                    stack.append((font_size, charset, dirctx, {k: v.copy() for k, v in eff.items()}, i, True))
                    i += 1
                else:
                    i = j
                continue
            ...
            if c == '{':
                # 6th element: is_caption_group (True only when entering {\*} that contains \shptxt)
                stack.append((font_size, charset, dirctx, {k: v.copy() for k, v in eff.items()}, i, False))
                i += 1
            elif c == '}':
                if text_parts:
                    text = self._clean_text(''.join(text_parts))
                ...
                if stack:
                    tos = stack.pop()
                    font_size, charset, dirctx, eff_snapshot = tos[0], tos[1], tos[2], tos[3]
                    eff = {k: v.copy() for k, v in eff_snapshot.items()}
                    if len(tos) >= 6 and tos[5]:
                ...
                if m:
                    word = data[i:m.end()].strip()
                    if (word.startswith(r'\shptxt') or word.startswith(r'\dptxbxtext')) and any(len(tos) >= 6 and tos[5] for tos in stack):
