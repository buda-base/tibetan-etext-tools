...
1. **Parse RTF** - Extract text with font information
   - Handles RTF escape sequences: `\'XX` (hex characters), `\{`, `\}`, `\\`
   - Skips metadata sections: `{\info`, `{\fonttbl`, `{\colortbl`, etc.
   - Properly handles escaped braces in metadata to prevent extraction
...
### RTF Parser Bug Fixes (2026-02-13)
- **Fixed Metadata Extraction Bug**: RTF metadata fields (author, subject, operator) were incorrectly being extracted as content
  - **Root Cause**: The `{\info` block skip logic didn't handle escaped braces (`\{` and `\}`) correctly
  - **Impact**: Metadata like "gjj.51.net", "???", "tenzin delek" was appearing in output XML files
  - **Solution**: Updated `basic_rtf.py` to properly skip escaped characters when counting braces in metadata sections
  - **Files Modified**: `basic_rtf.py` (lines 318-333)
...
### Escape Sequences
The RTF parser handles the following escape sequences:

| Sequence | Meaning | Example |
|----------|---------|---------|
| `\'XX` | Hexadecimal character (XX = 2-digit hex) | `\'cb\'ce\'cc\'e5` = 宋体 (SimSun in GB2312) |
| `\{` | Literal opening brace `{` | Used in text content |
| `\}` | Literal closing brace `}` | Used in text content (e.g., in `{\info{\title \}$}`) |
| `\\` | Literal backslash `\` | Used in text content |
| `\~` | Non-breaking space | Preserved as U+00A0 |
| `\-` | Optional hyphen | Ignored (soft hyphen) |
| `\uN` | Unicode character (N = decimal codepoint) | `\u2026` = … (ellipsis) |

### Metadata Sections
The following RTF sections are automatically skipped and not included in output:
- `{\info` - Document metadata (title, author, subject, operator, etc.)
- `{\fonttbl` - Font table definitions
- `{\colortbl` - Color table definitions
- `{\stylesheet` - Style definitions
- `{\header` / `{\footer` - Header and footer content
- `{\*\generator` - Generator information
- Other internal RTF structures
...
- **Cleanup**: Automatically removes lines containing only dashes and spaces (e.g., "- - - - -")
