...
4. **Normalize Text** - Fix flying vowels, Unicode normalization
   - **Dedris conversion error fixes** - Corrects U+0F84 → U+0F04 for head marks
...
- `tibetan_text_fixes.py` - Tibetan-specific text fixes
  - `fix_dedris_conversion_errors()` - Fixes U+0F84 → U+0F04 for head marks
  - `fix_flying_vowels_and_linebreaks()` - Fixes flying vowels and line breaks
  - `fix_ascii_to_tibetan()` - ASCII to Tibetan character mapping
- `FIX_APOSTROPHE_CONVERSION.md` - Documentation for head mark fix
...
- **Dedris Head Mark Fix** (2026-02-16): Fixed apostrophe `'` in Dedris-a font conversion
  - **Issue**: Apostrophe was incorrectly converted to U+0F84 (྄ HALANTA mark)
  - **Fix**: Now correctly converts to U+0F04 (༄ head mark)
  - **Result**: Tibetan opening mark `༄༔` now displays correctly
  - See `FIX_APOSTROPHE_CONVERSION.md` for details
