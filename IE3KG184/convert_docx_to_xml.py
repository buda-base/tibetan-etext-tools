#!/usr/bin/env python3
"""
DOCX to TEI XML Converter for IE3KG184

This script converts DOCX files to TEI XML format using the BasicDOCX parser.
IE3KG184 DOCX files are already in Unicode, so no Dedris conversion is needed.

IE3KG184 reads DOCX files from: toprocess/{IE_ID}-{VE_ID}/

Usage:
    python convert_docx_to_xml.py                          # Convert all DOCX files
    python convert_docx_to_xml.py --single IE3KG184-VE5CNxxx/file.docx  # Convert single file
    python convert_docx_to_xml.py --single IE3KG184-VE5CNxxx/file.docx --sequence 1  # With specific sequence
"""
...
def convert_docx_to_tei(docx_path: Path, ve_id: str, sequence: int) -> str:
    ...
    IE3KG184 DOCX files are already Unicode - no Dedris conversion is needed.
...
def convert_single_file(relative_path: str, sequence: int = 1):
    ...
    Args:
        relative_path: Path to DOCX file relative to toprocess folder
                      e.g., "IE3KG184-VE5CNxxx/file.docx"
