...
class BasicDOCX:
    ...
    def _extract_font_from_rpr(self, rpr_elem) -> tuple:
        ...
        if rfonts is not None:
            # For Dedris fonts, prioritize ascii/hAnsi over cs
            # The cs attribute is often set incorrectly to Dedris-vowa
            # while ascii/hAnsi correctly specify the variant (Dedris-a, Dedris-b, etc.)
            cs_font = rfonts.get(_get_tag('w', 'cs')) or rfonts.get('cs')
            ascii_font = rfonts.get(_get_tag('w', 'ascii')) or rfonts.get('ascii')
            hansi_font = rfonts.get(_get_tag('w', 'hAnsi')) or rfonts.get('hAnsi')
            
            # If ascii or hAnsi is a Dedris font, use it (prioritize over cs)
            if ascii_font and 'dedris' in ascii_font.lower():
                font_name = ascii_font
            elif hansi_font and 'dedris' in hansi_font.lower():
                font_name = hansi_font
            else:
                # Default priority: cs > ascii > hAnsi
                font_name = cs_font or ascii_font or hansi_font
