...
Usage:
    ...
    python convert.py --single IE1AB2-VE3KG59/file.docx  # Convert single file
"""
...
from tibetan_text_fixes import fix_hi_tag_spacing
...
def convert_docx_to_tei(docx_path: Path, ve_id: str, sequence: int, folder_name: str = None) -> str:
...
def convert_single_file(relative_path: str):
    ...
    try:
        tei_xml = convert_docx_to_tei(docx_path, ve_id, sequence, folder_name)
    except Exception as e:
    ...
    copy_sources_to_output(folder_name, [docx_path])
...
def convert_all_files():
    ...
    total_files = sum(len(info['files']) for info in docx_by_ve.values())
    ...
    for ve_id in natsorted(docx_by_ve.keys()):
        ve_info = docx_by_ve[ve_id]
        docx_files = ve_info['files']
        ...
        if converted_files:
            copy_sources_to_output(folder_name, converted_files)
