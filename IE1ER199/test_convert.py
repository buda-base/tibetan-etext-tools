#!/usr/bin/env python3
"""Test script for convert_derge.py"""
...
def test_get_ut_id():
    ...
    assert get_ut_id('VE1ER148') == 'UT1ER148_0001', "UT ID generation incorrect"
    assert get_ut_id('VE1ER199') == 'UT1ER199_0001', "UT ID generation incorrect"
...
def test_process_annotations():
    ...
    result = process_annotations('{D1}text')
    assert '<milestone xml:id="D1" unit="section"/>' in result, "D1 milestone not converted"
    
    # Test Derge milestone with sub-index {D1-1}
    result = process_annotations('{D1-1}text')
    assert '<milestone xml:id="D1-1" unit="section"/>' in result, "D1-1 milestone not converted"
...
[1a.1]
[1b]
[1b.1]{D1}༄༅༅། །རྒྱ་གར་སྐད་དུ།
[1b.2]རྣམས་ཡང་དག་རབ་བཅད་ཅིང་།
[2a]
[2a.1]{D1-1}དཀོན་མཆོག་གསུམ་ལ་ཕྱག་འཚལ་ལོ།
"""
    ...
    assert parsed['milestones'][0]['id'] == 'D1', "First milestone should be D1"
    assert parsed['milestones'][1]['id'] == 'D1-1', "Second milestone should be D1-1"
    
    # Check line content
    assert len(parsed['pages'][1]['lines']) == 2, "Page 1b should have 2 lines"
    assert '{D1}' in parsed['pages'][1]['lines'][0]['content'], "Line content should contain {D1}"
...
def test_generate_tei_body():
    ...
    parsed_data = {
        'pages': [
            ...
            },
            {
                'page_num': '1b',
                'lines': [
                    {'line_num': '1', 'content': '{D1}༄༅༅། །རྒྱ་གར་སྐད་དུ།'},
                    {'line_num': '2', 'content': 'རྣམས་ཡང་དག་རབ་བཅད་ཅིང་།'}
                ]
            }
        ],
        'milestones': []
    ...
    assert '<milestone xml:id="D1" unit="section"/>' in body, "Milestone not converted"
...
def test_full_conversion():
    ...
    sources_text = test_input / 'sources' / 'text'
    toprocess = test_input / 'toprocess'
    sources_text.mkdir(parents=True, exist_ok=True)
...
[1a.1]
[1b]
[1b.1]{D1}༄༅༅། །རྒྱ་གར་སྐད་དུ། བི་ན་ཡ་བསྟུ།
[1b.2]བོད་སྐད་དུ། འདུལ་བ་གཞི།
[2a]
[2a.1]{D1-1}དཀོན་མཆོག་གསུམ་ལ་ཕྱག་འཚལ་ལོ།
"""
    
    source_file = sources_text / '001_test_volume.txt'
    with open(source_file, 'w', encoding='utf-8') as f:
        f.write(test_content)
    
    # Create test toprocess folder (VE identifier)
    ve_folder = toprocess / 'IE1ER199-VE1ER148'
    ve_folder.mkdir(parents=True, exist_ok=True)
    
    # Run conversion
    convert_ie1er199(str(test_input), str(test_output))
    
    # Check outputs
    assert test_output.exists(), "Output directory not created"
    
    # Check sources
    sources_txt = test_output / 'sources' / 'VE1ER148' / '001_test_volume.txt'
    assert sources_txt.exists(), f"Source txt file not copied: {sources_txt}"
    
    # Check archive XML
    archive_xml = test_output / 'archive' / 'VE1ER148' / 'UT1ER148_0001.xml'
    assert archive_xml.exists(), f"Archive XML not created: {archive_xml}"
    
    # Check CSV
    csv_file = test_output / 'IE1ER199.csv'
    ...
    with open(archive_xml, 'r', encoding='utf-8') as f:
        ...
        assert '<milestone xml:id="D1" unit="section"/>' in xml_content, "Milestone not in XML"
        assert 'xml:space="preserve"' in xml_content, "xml:space preserve not in XML"
        assert 'src_sha256' in xml_content, "SHA256 not in XML"
        assert 'VE1ER148' in xml_content, "VE ID not in XML"
        assert 'UT1ER148_0001' in xml_content, "UT ID not in XML"
    
    # Verify CSV content
    with open(csv_file, 'r', encoding='utf-8') as f:
        csv_content = f.read()
        assert 'D1' in csv_content, "D1 milestone not in CSV"
        assert 'D1-1' in csv_content, "D1-1 milestone not in CSV"
...
def test_get_ve_ids_from_toprocess():
    ...
    (test_dir / 'IE1ER199-VE1ER148').mkdir(exist_ok=True)
    (test_dir / 'IE1ER199-VE1ER149').mkdir(exist_ok=True)
    (test_dir / 'IE1ER199-VE1ER150').mkdir(exist_ok=True)
    
    volumes = get_ve_ids_from_toprocess(test_dir)
    
    assert len(volumes) == 3, f"Expected 3 volumes, got {len(volumes)}"
    assert volumes[0]['ve_id'] == 'VE1ER148', "First VE ID incorrect"
    assert volumes[1]['ve_id'] == 'VE1ER149', "Second VE ID incorrect"
    assert volumes[2]['ve_id'] == 'VE1ER150', "Third VE ID incorrect"
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("Running Derge Kangyur Converter Tests")
    print("=" * 60)
