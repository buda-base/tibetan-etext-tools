#!/usr/bin/env python3
"""
Convert PDF files from IE3KG690 to TEI XML format.
...
def get_ve_ids_from_toprocess(input_path: Path) -> list:
    ...
    for folder in toprocess_path.iterdir():
        if folder.is_dir() and folder.name.startswith('VE'):
            # Handle both formats: IE3KG690-VE1ER566 and VE1ER566
...
def get_pdf_files(input_path: Path, ve_ids: list) -> list:
    ...
    pdf_files = []
    for ve_id in ve_ids:
        # Try both formats: IE3KG690-VE1ER566 and VE1ER566
...
def convert_IE3KG690(input_path: str, output_path: str):
    """
    Main conversion function for IE3KG690.
    
    Args:
        input_path: Path to input folder (IE3KG690_INPUT)
        output_path: Path to output folder (IE3KG690)
    """
...
def main():
    if len(sys.argv) != 3:
        ...
        print(f"\nExample:")
        print(f"  python {sys.argv[0]} /path/to/IE3KG690_INPUT /path/to/IE3KG690")
        sys.exit(1)
    
    input_folder = sys.argv[1]
    output_folder = sys.argv[2]
    
    convert_IE3KG690(input_folder, output_folder)
