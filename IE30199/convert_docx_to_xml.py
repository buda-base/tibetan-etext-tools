#!/usr/bin/env python3
"""
DOCX to TEI XML Converter for IE30199

This script converts DOCX files to TEI XML format using the BasicDOCX parser.
IE30199 DOCX files are already in Unicode, so no Dedris conversion is needed.

IE30199 has DOCX files directly in toprocess/IE30199-{VE_ID}/ folders.

Usage:
    python convert_docx_to_xml.py                          # Convert all DOCX files
    python convert_docx_to_xml.py --single IE30199-VE5CN21/01.docx  # Convert single file
    python convert_docx_to_xml.py --single IE30199-VE5CN21/01.docx --sequence 1  # With specific sequence
"""
...
# Note: IE30199 DOCX files are already in Unicode, so Dedris conversion is not used
...
def convert_docx_to_tei(docx_path: Path, ve_id: str, sequence: int) -> str:
    ...
    for stream in streams:
        ...
        # IE30199 DOCX files are already in Unicode - no Dedris conversion needed
        if not text:
            continue
...
def convert_single_file(relative_path: str, sequence: int = 1):
    ...
    # Extract VE ID from parent folder name (e.g., IE30199-VE5CN21 -> VE5CN21)
...
def convert_all_files():
    ...
    logger.info(f"Source: {TOPROCESS_DIR}")
