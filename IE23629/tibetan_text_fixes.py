...
# =============================================================================
# Space Within Tibetan Syllables (RTF Run Boundaries)
# =============================================================================

def remove_spaces_within_tibetan_syllables(text: str) -> str:
    """
    Remove erroneous spaces that appear between Tibetan script characters.

    Some RTF files emit a new text run (e.g. font/format change) around each
    character or syllable part, and the parser may capture a space at run
    boundaries. That produces output like "ཚ ད་མ་ར ྣ མ་" instead of
    "ཚད་མ་རྣམ་". This function removes any space that is between two
    Tibetan characters (U+0F00-U+0FFF), so syllables are rejoined correctly.

    Space between two shad marks (། །, U+0F0D-U+0F11) is preserved for
    sentence separation.

    Args:
        text: Input text that may contain spaces inside Tibetan syllables

    Returns:
        Text with spaces between Tibetan characters removed (shad-space-shad kept)
    """
    if not text:
        return text
    ...
    def replace_fn(match: re.Match) -> str:
        ...
        # Preserve space between two shad marks (sentence separators)
        if (
            len(left) == 1
            and len(right) == 1
            and 0x0F0D <= ord(left) <= 0x0F11
            and 0x0F0D <= ord(right) <= 0x0F11
        ):
            return match.group(0)
        return left + right

    # One or more spaces between two Tibetan block characters
    pattern = rf"([\u0f00-\u0fff]) +([\u0f00-\u0fff])"
    while True:
