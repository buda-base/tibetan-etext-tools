...
Contain subdirectory named KHANDO
Input structure:
    {IE_ID}/toprocess/{IE_ID-VE_ID}/KHADON/*.rtf
    Example: IE29198/toprocess/IE29198-VE3KG69/KHADON/*.rtf

Output structure:
    Archive (flat): {IE_ID}_output/archive/{VE_ID}/UT{suffix}_{FILE_NUM}.xml
    Sources (nested): {IE_ID}_output/sources/{IE_ID-VE_ID}/KHADON/*.rtf
...
from tibetan_text_fixes import (
    ...
    fix_hi_tag_spacing,
    remove_spaces_within_tibetan_syllables,
    count_tibetan_chars,
)
...
def get_volume_folders(ie_id: str, toprocess_dir: Path) -> list:
    ...
    Each subfolder is named {IE_ID-VE_ID}; under that, KHADON contains *.rtf.
    ...
    for folder in toprocess_dir.iterdir():
        ...
        # Folder name is IE_ID-VE_ID (e.g. IE29198-VE3KG69)
        name = folder.name
        ...
        if khadon.is_dir():
            ...
            if rtf_files:
                volumes.append((ve_id, "KHADON", khadon, None))
...
def convert_rtf_to_tei(rtf_path: Path, ie_id: str, ve_id: str, ut_id: str, src_path: str) -> str:
    ...
    # Remove erroneous spaces within Tibetan syllables (e.g. "ཚ ད་མ་" -> "ཚད་མ་")
    # caused by RTF run boundaries between each character
    body_content = remove_spaces_within_tibetan_syllables(body_content)
...
def process_volume(ie_id: str, ve_id: str, volume_id: str, volume_folder: Path, 
    ...
    try:
        ...
        # Archive: flat under archive/{VE_ID}/; sources: nested under sources/{IE_ID-VE_ID}/KHADON/
