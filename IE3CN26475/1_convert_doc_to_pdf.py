...
IE3CN4059 has DOC files in nested subfolders (subfolder names have .doc extension):
  toprocess/{IE_ID}-{VE_ID}/{subfolder}/*.doc

Usage:
    python 1_convert_doc_to_pdf.py           # Convert all DOC files
    python 1_convert_doc_to_pdf.py --single IE3CN4059-VE5CN1124/subfolder/file.doc
"""
...
def get_all_word_files() -> list:
    """Get all DOC files from toprocess/{IE_ID}-{VE_ID}/{subfolder}/*.doc folders.
    
    IE3CN4059 has a nested structure where DOC files are inside subfolders
    that have .doc in their names (but are actually folders, not files).
    ...
    for ve_folder in TOPROCESS_DIR.iterdir():
        if ve_folder.is_dir() and ve_folder.name.startswith(f"{IE_ID}-"):
            ...
            if ve_id:
                # Recursively find all .doc FILES (not folders) within VE folder
                doc_files = []
                for doc_path in ve_folder.rglob("*.doc"):
                    # Only include actual files, not directories with .doc in name
                    if doc_path.is_file():
...
def sanitize_filename(name: str) -> str:
    """Remove trailing dots and other invalid characters from filename."""
...
def convert_doc_to_pdf(doc_path: Path, ve_id: str, word_app) -> Path:
    ...
    # Sanitize filename to handle malformed names like "file..doc"
    clean_stem = sanitize_filename(doc_path.stem)
    pdf_filename = clean_stem + ".pdf"
    pdf_path = pdf_ve_dir / pdf_filename
    
    doc = None
    try:
        # Open with parameters to suppress dialogs:
...
def convert_single_file(relative_path: str):
    ...
    Args:
        relative_path: Path to DOC file relative to toprocess/
                      e.g., "IE3CN4059-VE5CN1124/subfolder/file.doc"
    """
    ...
    # Find the VE folder (first level under toprocess)
...
def convert_all_files():
    ...
    try:
        word_app = win32com.client.Dispatch("Word.Application")
        word_app.Visible = True  # Make visible so user can see/dismiss any dialogs
        word_app.DisplayAlerts = 0  # wdAlertsNone - suppress alerts
