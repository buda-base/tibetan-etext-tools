...
def remove_non_tibetan(text: str) -> str:
    ...
    # First, protect numbers inside parentheses by temporarily replacing them
    # Match both ASCII parentheses () and full-width parentheses （）
    protected_numbers = []
    def protect_parenthesized_numbers(match):
        protected_numbers.append(match.group(0))
        return f"__PROTECTED_{len(protected_numbers)-1}__"
    
    text_protected = re.sub(r'[(\uFF08]\d+[)\uFF09]', protect_parenthesized_numbers, tibetan_only)
    
    # Remove all characters outside the Tibetan Unicode range, whitespace, and parentheses
    tibetan_range = re.sub(r'[^\u0F00-\u0FFF\s()\uFF08\uFF09]', '', text_protected)
    
    # Restore protected numbers
    for i, protected in enumerate(protected_numbers):
        tibetan_range = tibetan_range.replace(f"__PROTECTED_{i}__", protected)
