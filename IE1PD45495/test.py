#!/usr/bin/env python3
"""
RTF to TEI XML Converter - Single File or Batch Processing
...
Usage:
    # Process a single RTF file (no folder structure needed):
    python test.py --file volume_017_139.rtf
    
    # Process with custom VE ID:
    python test.py --file volume_017_139.rtf --ve-id VE3KG466
    
    # Process with custom output directory:
    python test.py --file volume_017_139.rtf --output-dir my_output
    
    # Batch mode - Process all collections (requires folder structure):
    python test.py
    
    # Batch mode - Process specific collection:
    python test.py --ie-id IE1PD45495
    
    # Batch mode - Adjust worker count:
    python test.py --workers 4

Folder Structure (Batch Mode Only):
    Input:  {IE_ID}/sources/{VE_ID}/{collection_name}/rtfs/{VOL_ID}/*.rtf
    Output: {IE_ID}_output/archive/{VE_ID}/UT{suffix}_{index}.xml
"""
...
# Text Cleaning
from text_cleaning import remove_page_markers, remove_embed_objects, normalize_guillemets
...
def dedris_to_unicode(text: str, font_name: str, stats: dict = None) -> str:
    """
    Legacy Tibetan font to Unicode conversion with RTF artifact cleaning.
    """
    ...
    # 1. PRE-CLEANING (Project Specific)
    text = remove_page_markers(text)
    text = remove_embed_objects(text)
    
    # 2. PROTECT NON-TIBETAN CONTENT
    # If the stream is just punctuation, numbers, or dots, don't attempt conversion
    if re.fullmatch(r'[\s0-9.,;:!?-–—()\[\]{}«»/·…¡¤]*', text):
        return text

    font_lower = font_name.lower() if font_name else ''
    
    # 3. SKIP UNICODE FONTS
    # Don't convert if already in Unicode (TibetanMachineUnicode, etc.)
    if 'unicode' in font_lower or 'jomolhari' in font_lower or 'monlam' in font_lower:
        return text
    
    # 4. ATTEMPT CONVERSION FOR LEGACY FONTS
    # Try to convert fonts that might contain legacy Tibetan encoding
    try:
        # First, try with the original font name
        result = convert_string(text, font_name, stats)
        
        # If that fails and font is Times New Roman, try common legacy Tibetan fonts
        if result is None and font_lower in ('times new roman', 'times'):
            # Try TibetanMachine (common legacy font)
            result = convert_string(text, "TibetanMachine", stats)
            
            # If still None, try other common legacy fonts
            if result is None:
                for fallback_font in ["TibetanMachineWeb", "DDC_Uchen", "Dedris", "eDedris"]:
                    result = convert_string(text, fallback_font, stats)
                    if result is not None:
                        break
        
        if result is None:
            # Log font name for future mapping updates
            if font_name not in stats.get("unhandled_fonts", {}):
                logger.debug(f"Unhandled font: '{font_name}'")
            return text
...
def convert_rtf_to_tei(rtf_path: Path, ve_id: str, ut_id: str, ie_id: str, 
    ...
    # STAGE 1: RTF Parsing
    # =========================================================================
    logger.info(f"  Stage 1: Parsing RTF file: {rtf_path.name}")
    parser = BasicRTF()
    ...
    for stream in streams:
        ...
        # Handle breaks
        if stream.get("type") in ("par_break", "line_break", "cell_break"):
        ...
        # Fix double backslashes from RTF parser
        # RTF parser may pass raw escape sequences with doubled backslashes
...
def process_single_file(rtf_path: Path, ve_id: str, ie_id: str, output_dir: Path):
    """
    ...
    Args:
        ...
        ie_id: Image Entity ID (e.g., IE1PD45495)
    ...
    try:
        ...
        logger.info(f"Processing: {rtf_path.name}")
        logger.info(f"  VE ID: {ve_id}")
        logger.info(f"  UT ID: {ut_id}")
        
        # Convert RTF to TEI using 4-stage pipeline
        sources_dir = rtf_path.parent
        tei_xml = convert_rtf_to_tei(rtf_path, ve_id, ut_id, ie_id, sources_dir)
        
        # Create output directory
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # Write XML file
        xml_path = output_dir / f"{ut_id}.xml"
        with open(xml_path, 'w', encoding='utf-8') as f:
            f.write(tei_xml)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RTF to TEI XML Converter - Process single file or batch collections"
    )
    parser.add_argument(
        "--file",
        type=Path,
        help="Process a single RTF file (e.g., volume_017_139.rtf)"
    )
    parser.add_argument(
        "--ve-id",
        default="VE3KG466",
        help="Volume Entity ID (default: VE3KG466)"
    )
    parser.add_argument(
        "--ie-id",
        default="IE1PD45495",
        help="Image Entity ID (default: IE1PD45495)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Output directory (default: current_dir/output)"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=INPUT_DIR,
        help=f"Input directory for batch mode (default: {INPUT_DIR})"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    logger.info("=" * 70)
    logger.info("RTF TO TEI XML CONVERTER")
    logger.info("=" * 70)
    
    # SINGLE FILE MODE
    if args.file:
        rtf_path = args.file if args.file.is_absolute() else Path.cwd() / args.file
        
        if not rtf_path.exists():
            logger.error(f"File not found: {rtf_path}")
            return
        
        if not rtf_path.suffix.lower() == '.rtf':
            logger.error(f"Not an RTF file: {rtf_path}")
            return
        
        output_dir = args.output_dir if args.output_dir else Path.cwd() / "output"
        
        logger.info(f"Mode: Single File")
        logger.info(f"Input: {rtf_path}")
        logger.info(f"Output: {output_dir}")
        logger.info("=" * 70)
        
        success = process_single_file(rtf_path, args.ve_id, args.ie_id, output_dir)
    ...
    if args.ie_id != "IE1PD45495":  # Only filter if user changed default
