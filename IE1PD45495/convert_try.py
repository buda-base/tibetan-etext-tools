...
ENABLE_FONT_CLASSIFICATION = True   # Stage 4: Font size classification (ENABLED)
...
def dedris_to_unicode(text: str, font_name: str, stats: dict = None) -> str:
    """
    Improvised Dedris to Unicode conversion with RTF artifact cleaning.
    """
    ...
    # 3. STRICT FONT FILTERING
    # Only convert if the font is explicitly Dedris/Legacy
    if not font_lower.startswith(('dedris', 'ededris')):
        return text

    try:
        # Convert using pytiblegenc
        ...
        if result is None:
            ...
            if font_name not in stats.get("unhandled_fonts", {}):
                logger.debug(f"Unhandled Dedris font: '{font_name}'")
            return text
