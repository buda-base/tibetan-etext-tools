...
def collapse_duplicate_tibetan_span_marks(text: str) -> str:
    """
    Collapse consecutive duplicate Tibetan combining marks (vowel signs, subjoined, etc.).

    PyMuPDF often duplicates a mark at span boundaries (e.g. ``གོ`` vs ``གོོ`` with two U+0F7B),
    which pdfminer/pytiblegenc typically does not. Repeated runs of the *same* mark in the
    Tibetan combining ranges are not valid orthography for these code points.

    This **only removes** extra copies of the same code point; it does **not** insert vowels.
    """
    if not text:
        return text
    return re.sub(
