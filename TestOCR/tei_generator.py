...
def post_process_body(body_content: str) -> str:
    ...
    # convert_markup_to_tei already inserts <lb/> between lines; the rule above adds another
    # after every newline, which yields <lb/><pb/> before page breaks — strip the duplicate.
    body_content = re.sub(r'<lb/>\s*<pb/>', r'<pb/>', body_content)
