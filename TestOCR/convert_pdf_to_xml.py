#!/usr/bin/env python3
"""
IE3KG693: PDF in sources/ to TEI XML (Dedris via pytiblegenc)

Input (default batch): PDFs only under **sources/<VE_ID>/** — volume ID is the **folder name**
(e.g. sources/VE1ER1074/*.pdf → output archive/VE1ER1074/). **toprocess is ignored** for which
volumes run; a mismatched folder like toprocess/IE3KG693-VE3KG667 does not affect VE1ER1074.

Optional: flat sources/*.pdf — use ``--assign-flat-toprocess`` to split those across
IE3KG693-VE* folders in toprocess; otherwise root PDFs are skipped (move into sources/<VE_ID>/).

toprocess is still checked only for optional matching .doc (IE3KG692-<same VE as source>) for SHA256.
Output: IE3KG693_output/archive/{VE_ID}/UT*.xml and sources/{VE_ID}/ (PDF + optional DOC).

Optional: matching .doc in toprocess/IE3KG693-{ve_id}/ for SHA256; otherwise SHA256 from PDF.

Usage:
    python convert_pdf_to_xml.py                    # Convert all PDFs (assigned to VEs)
    python convert_pdf_to_xml.py --ve VE1ER1074      # One VE only
    python convert_pdf_to_xml.py --single foo.pdf
    python convert_pdf_to_xml.py --single VE1ER1074/TI1751-01-001.pdf  # path under sources/
...
from config import (
    ...
    PDF_EXTRACT_REGION,
    PDF_EXTRACT_BACKEND,
    PDF_EXTRACT_TESS_DPI,
    PDF_EXTRACT_TESS_LANG,
    PDF_EXTRACT_TESS_CONFIG,
    ensure_directories,
    get_ut_id,
...
from pymupdf_extractor import extract_pdf_to_text_pymupdf
from tesseract_extractor import extract_pdf_to_text_tesseract
...
def setup_logging():
    ...
    ``basicConfig()`` is skipped if the root logger already has handlers (e.g. after
    another library configured logging). Use ``force=True`` (Py 3.8+) or clear handlers
    so INFO lines always reach stdout and the log file.
    """
    ensure_directories()
    ...
    log.info("→ setup_logging | log_file=%s", PDF_TO_XML_LOG)
    return log
...
def _file_only_logger(name: str) -> logging.Logger:
    """Logger that writes only to ``PDF_TO_XML_LOG`` (no console). For verbose dumps."""
    ...
    lg.propagate = False
    fmt = "%(asctime)s - %(levelname)s - %(message)s"
    fh = logging.FileHandler(PDF_TO_XML_LOG, encoding="utf-8")
    fh.setFormatter(logging.Formatter(fmt))
    lg.addHandler(fh)
    return lg


# Full PDF extract text is logged here only (not echoed to stdout/stderr).
_pdf_extract_body_logger = _file_only_logger(__name__ + ".pdf_extract_body")
...
PAGE_BREAK_STR = "ZZZZ"
# Single-line marker before <lb/> join; replaced by <pb/> so pages end …text\n<pb/>, not …<lb/><pb/>.
PB_LINE_PLACEHOLDER = "<<<PB>>>"
FONT_SIZE_FORMAT = "<fs:{}>"

# Tibetan tsheg (U+0F0B), vowel ུ (U+0F74), ASCII digits, fullwidth digits (U+FF10-U+FF19)
_PAGE_ARTIFACT_CHARS = r"\u0F0B\u0F740-9\uFF10-\uFF19\s"
# Whole-line footer: page artifacts + Latin letters (e.g. "Page", "iv", "A", "xii").
# ``strip_page_number_artifacts`` uses this set so letter footers strip like numeric ones.
_LINE_ONLY_CHARS = _PAGE_ARTIFACT_CHARS + "a-zA-Z"
_LINE_ONLY_PAGE_ARTIFACTS = re.compile(r"^[" + _LINE_ONLY_CHARS + r"]*$")


def _char_is_line_only_artifact(ch: str) -> bool:
    return bool(re.match(r"[" + _LINE_ONLY_CHARS + r"]", ch))


def _is_artifact_only_between_lb(segment: str) -> bool:
    """Only LINE_ONLY chars (same intent as the old between-<lb/> regex). Linear time."""
    if not segment:
        return False
    for ch in segment:
        if not _char_is_line_only_artifact(ch):
            return False
    return True


def _strip_artifact_blocks_between_lb_once(text: str) -> str:
    parts = text.split("<lb/>")
    if len(parts) < 2:
        return text
    kept = [parts[0]]
    ...
    while i < len(parts):
        seg = parts[i]
        if i + 1 < len(parts) and _is_artifact_only_between_lb(seg):
            i += 1
            continue
        kept.append(seg)
        i += 1
...
def _strip_artifact_blocks_between_lb(text: str) -> str:
    prev = None
    while prev != text:
        prev = text
        text = _strip_artifact_blocks_between_lb_once(text)
    return text


def _consume_pb_unit(s: str, i: int) -> int | None:
    """One unit: <lb/> \\s* [LINE_ONLY]* \\n? — returns end index, or None if not a unit at i."""
    if i + 5 > len(s) or s[i : i + 5] != "<lb/>":
        return None
    j = i + 5
    while j < len(s) and s[j] in " \t\r":
        j += 1
    k = j
    while k < len(s) and _char_is_line_only_artifact(s[k]):
        k += 1
    if k < len(s) and s[k] == "\n":
        k += 1
    return k
...
def _try_peel_one_unit_from_end(s: str) -> str | None:
    end = len(s)
    pos = end
    while True:
        pos = s.rfind("<lb/>", 0, pos)
        if pos < 0:
            return None
        if _consume_pb_unit(s, pos) == end:
            return s[:pos]


def _strip_all_pb_units_from_end(s: str) -> str:
    cur = s
    while True:
        nxt = _try_peel_one_unit_from_end(cur)
        if nxt is None:
            return cur
        cur = nxt


def _strip_pattern1_suffix_before_pb(S: str) -> str:
    """Same as old re.sub pattern 1: (unit)+ \\s*<lb/> \\s* before <pb/> -> newline."""
    s = S.rstrip()
    if len(s) < 5 or not s.endswith("<lb/>"):
        return S
    lb_pos = len(s) - 5
    j = lb_pos
    while j > 0 and s[j - 1] in " \t\r":
        j -= 1
    before = s[:j]
    rest = _strip_all_pb_units_from_end(before)
    if rest == before:
        return S
    return rest + "\n"


def _trim_segment_before_pb(seg: str) -> str:
    """Apply former pattern1 then pattern2 to the text chunk before each <pb/>."""
    s = _strip_pattern1_suffix_before_pb(seg)
    s = _strip_all_pb_units_from_end(s)
    return s


def strip_page_number_artifacts(text: str) -> str:
    """
    Remove PDF footer artifacts: page numbers, Latin letters (A-Z, a-z, e.g. folio labels),
    long tsheg runs, standalone ུ, etc.

    Footers often sit *between* two ``<lb/>`` markers (before the next line), with body
    text and more ``<lb/>`` lines before ``<pb/>`` — not immediately before ``<pb/>``.
    Those blocks are removed first. Remaining artifact runs right before ``<lb/><pb/>``
    or ``<pb/>`` are then stripped.

    Implemented without nested-quantifier regex on the full string (avoids hangs on large PDFs).
    """
    text = _strip_artifact_blocks_between_lb(text)
    parts = text.split("<pb/>")
    for i in range(len(parts) - 1):
        parts[i] = _trim_segment_before_pb(parts[i])
    return "<pb/>".join(parts)


def _mark_footer_strip_exception_lines(lines: list[str]) -> list[str]:
    """
    TEMP: Mark raw lines (before <lb/> insertion) so strip_page_number_artifacts skips them.
    Regex on <lb/>-wrapped text failed when lines had <fs:> tags or different newlines.
    Delete with strip_page_number_artifacts_keep_exceptions when extractor is fixed.
    """
    start, end = "\uE000", "\uE001"
    out = list(lines)
    for i, line in enumerate(out):
        if start in line:
            continue
        s = line.rstrip()
        plain = re.sub(r"<fs:\d+>", "", s).strip()
        plain = plain.replace("\r", "")
        if not plain:
            continue
        # Verse/citation lines with >> must not be treated as footer-only strips (tsheg is
        # in _LINE_ONLY_CHARS; between-<lb/> regex can otherwise damage these lines).
        if ">>" in plain:
            out[i] = start + s + end
            continue
        # Digit-only lines are never protected: they are always footers (page numbers, folios).
        # Protecting them caused 3+ digit footers (e.g. 522) before a >> line to survive
        # stripping, incorrectly adding footers to pages that should have none.
        if re.fullmatch(r"[0-9\uFF10-\uFF19]+", plain):
            continue
        if re.fullmatch(r"[a-zA-Z\s]+", plain):
            if len(plain) > 10 or any(len(w) > 8 for w in plain.split()):
                out[i] = start + s + end
    return out


def strip_page_number_artifacts_keep_exceptions(text: str) -> str:
    """
    TEMP: Runs strip_page_number_artifacts and removes private-use markers from
    :func:`_mark_footer_strip_exception_lines`. Delete both when PDF extraction is fixed.
    """
...
def extract_pdf_to_text(pdf_path: Path) -> str:
    """Extract text using ``config.PDF_EXTRACT_BACKEND`` (pytiblegenc, pymupdf, or tesseract)."""
    backend = (PDF_EXTRACT_BACKEND or "pytiblegenc").strip().lower()

    """from pytiblegenc.font_utils import get_glyph_db_path, build_font_hash_index_from_csv, identify_pdf_fonts_from_db
    ...
    print(f"Extracting text from {pdf_path.name} using {backend} backend")
    try:
        if backend == "pymupdf":
            text = extract_pdf_to_text_pymupdf(
                pdf_path,
                region=PDF_EXTRACT_REGION,
                page_break_str=f"\n{PAGE_BREAK_STR}\n",
                font_size_format=FONT_SIZE_FORMAT,
            )
            logger.info(f"Extracted {len(text)} characters from {pdf_path.name}")
        elif backend == "tesseract":
            text = extract_pdf_to_text_tesseract(
                pdf_path,
                region=PDF_EXTRACT_REGION,
                page_break_str=f"\n{PAGE_BREAK_STR}\n",
                dpi=PDF_EXTRACT_TESS_DPI,
                lang=PDF_EXTRACT_TESS_LANG,
                tesseract_config=PDF_EXTRACT_TESS_CONFIG,
            )
            logger.info(f"Extracted {len(text)} characters from {pdf_path.name}")
        else:
            text = pdf_to_txt(
        ...
        _pdf_extract_body_logger.info(f"Extracted {len(text)} characters from {pdf_path.name}:\n{text}")
        return text
...
def convert_markup_to_tei(text: str) -> str:
    ...
    text = re.sub(PAGE_BREAK_STR, PB_LINE_PLACEHOLDER, text)

    text = "<pb/>\n" + text

    lines = text.split("\n")
    lines = _mark_footer_strip_exception_lines(lines)
    result = []

    for i, line in enumerate(lines):
        line = line.rstrip()
        if i > 0:
            # Page break line: newline only, so last line of page is …text\n<pb/>, not …<lb/><pb/>.
            if line.strip() == PB_LINE_PLACEHOLDER:
                result.append("\n")
            else:
                result.append("\n<lb/>")
        result.append(line)
    ...
    text = text.replace(PB_LINE_PLACEHOLDER, "<pb/>")

    text = strip_page_number_artifacts_keep_exceptions(text)
    ...
    text = re.sub(r"(<lb/>[\s\n]*)+</hi>", r"</hi>", text)
    text = re.sub(r"<lb/>[\s\n]*<pb", r"<pb", text)
    # Catch any remaining <lb/> immediately before <pb/> (e.g. after strip or hi fixes).
    text = re.sub(r"<lb/>\s*\n\s*<pb/>", r"\n<pb/>", text)
    text = re.sub(r"<lb/>\s*<pb/>", r"<pb/>", text)
...
def find_source_doc_file(pdf_path: Path, ve_id: str) -> Path:
    ...
    logger.debug(f"→ find_source_doc_file | pdf={pdf_path.name} ve_id={ve_id}")
...
def main():
    ...
    logger.debug(
        "CLI: single=%r ve=%r sequence=%s assign_flat=%s no_font_tags=%s no_normalization=%s",
        args.single,
        args.ve,
        args.sequence,
        args.assign_flat_toprocess,
        args.no_font_tags,
