...
Usage:
    ...
    python 2_convert_rtf_to_xml.py --limit 4 # Convert only first 4 files per volume
    python 2_convert_rtf_to_xml.py --single VE5CN239/file.rtf  # Convert single file
    python 2_convert_rtf_to_xml.py --single VE5CN239/file.rtf --sequence 4  # Convert with specific sequence
"""
...
from config import (
    IE_ID, TOPROCESS_DIR, RTF_DIR, OUTPUT_DIR, ARCHIVE_DIR, SOURCES_OUTPUT_DIR,
...
def find_source_word_file(rtf_filename: str, ve_id: str) -> Path:
    """
    Find the source DOC or DOCX file in toprocess folder structure.
    
    Args:
        rtf_filename: Name of RTF file (e.g., "6内容.rtf")
        ve_id: VE ID (e.g., "VE5CN239")
        
    Returns:
        Path to DOC/DOCX file or None if not found
    """
    base_name = Path(rtf_filename).stem
    ...
    # Search in subfolders - try .doc first, then .docx
    for subfolder in ve_folder.iterdir():
        if subfolder.is_dir():
            ...
            # Try .docx if .doc not found
            docx_path = subfolder / f"{base_name}.docx"
            if docx_path.exists():
                return docx_path
...
def convert_rtf_to_tei(rtf_path: Path, ve_id: str, sequence: int) -> str:
    ...
    word_path = find_source_word_file(rtf_path.name, ve_id)
    
    if not word_path:
        logger.warning(f"Source DOC/DOCX file not found for {rtf_path.name}, using RTF for SHA256")
        word_path = rtf_path
...
def copy_sources_to_output(ve_id: str, rtf_files: list):
    """Copy source files (DOC/DOCX and RTF) to output directory."""
    ...
    for rtf_path in rtf_files:
        ...
        # Find source DOC or DOCX file in toprocess folder
        word_path = find_source_word_file(rtf_path.name, ve_id)
...
def convert_all_files(limit: int = None):
    """Convert RTF files to TEI XML.
    
    Args:
        limit: If specified, only process the first N files per volume
    """
    logger.info("=" * 60)
...
def main():
    ...
    parser.add_argument("--sequence", type=int, default=1, help="Sequence number for UT ID (used with --single)")
    parser.add_argument("--limit", "-l", type=int, metavar="N", help="Process only first N files per volume")
    ...
    if args.single:
        ...
        convert_all_files(limit=args.limit)
