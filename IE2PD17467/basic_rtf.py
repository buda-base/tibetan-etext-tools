...
_RE_SECT = re.compile(r'\\sect(?![a-zA-Z])\s?')  # Section/page break
...
class BasicRTF:
    ...
    def _extract_textbox_content(self, group_content: str) -> list:
        """
        Extract text content from textbox groups (dptxbxtext or shptxt).
        
        These are text boxes inside shapes that contain actual document text.
        The text inside needs to be parsed like regular RTF content.
        
        Handles two patterns:
        1. {\shptxt \ltrpar ... content ...} - brace BEFORE keyword
        2. {\dptxbxtext\ltrpar ... content ...} - brace BEFORE keyword
        
        Args:
            group_content: The full content of the {\*\shpinst...} or {\*\do...} group
        ...
        # Pattern: {\shptxt ... } or {\dptxbxtext ... }
        # The { comes BEFORE the keyword
        
        pos = 0
        while pos < len(group_content):
            # Find textbox markers
            idx_shptxt = group_content.find(r'{\shptxt', pos)
            idx_dptxbx = group_content.find('dptxbxtext', pos)
            
            # Find nearest marker
            idx = -1
            marker_len = 0
            if idx_shptxt != -1 and (idx_dptxbx == -1 or idx_shptxt < idx_dptxbx):
                idx = idx_shptxt
                marker_len = len(r'{\shptxt')
            elif idx_dptxbx != -1:
                # Find the { before dptxbxtext
                brace_pos = group_content.rfind('{', pos, idx_dptxbx)
                if brace_pos != -1:
                    idx = brace_pos
                    marker_len = idx_dptxbx - brace_pos + len('dptxbxtext')
                else:
                    idx = idx_dptxbx
                    marker_len = len('dptxbxtext')
            ...
            while k < len(group_content):
                if group_content[k] == '{':
                    brace_level += 1
                elif group_content[k] == '}':
                    brace_level -= 1
                    if brace_level == 0:
                        break
                k += 1
            
            if k >= len(group_content):
                pos = idx + 1
                continue
            
            # Extract the textbox RTF content (from after the { to before the })
            textbox_rtf = group_content[idx:k+1]
            
            # Parse this mini-RTF content to extract text
            text = self._parse_textbox_rtf(textbox_rtf)
            if text:
                results.append(text)
    ...
    def _parse_textbox_rtf(self, rtf_content: str) -> str:
        r"""
        Parse RTF content from a textbox and extract plain text.
        
        This is a simplified parser for textbox content, extracting:
        ...
        Args:
            rtf_content: RTF content from inside a textbox (may include outer braces)
        ...
        # Track brace level - start at 0, process content at level 1 (inside outer braces)
        # and recursively handle deeper levels
        brace_level = 0
        ...
        while i < data_len:
            ...
            # Only process content at level 1 or deeper (inside some braces)
            if brace_level < 1:
                i += 1
                continue
            ...
            if c == '\\':
                ...
                if m:
                    try:
                        text_parts.append(chr(int(m.group(1), 16)))
                    except:
                        pass
                    i = m.end()
                    continue
                ...
                if data.startswith(r'\par', i) and (i + 4 >= data_len or not data[i+4].isalpha()):
                    text_parts.append('\n')
                    i += 4
                ...
                # Handle line break
                if data.startswith(r'\line', i) and (i + 5 >= data_len or not data[i+5].isalpha()):
                    text_parts.append('\n')
                    i += 5
                    if i < data_len and data[i] == ' ':
                        i += 1
            ...
            # Regular character (skip newlines and carriage returns from RTF formatting)
            if c not in '\r\n':
                text_parts.append(c)
            i += 1
    ...
    def parse_file(self, file_path, show_progress=True):
        ...
        # Track header/footer context - (type, stack_level) or None
        # When we see \headerl, \headerr, etc., mark all content until we exit that stack level
        hf_context = None  # ('header', stack_level) or ('footer', stack_level) or None
        
        # Header/footer keywords (appearing inside groups like {\headerl ...})
        # Note: single backslash in string (not raw string with \\)
        hf_keywords = {
            '\\headerl': 'header', '\\headerr': 'header', '\\headerf': 'header',
            '\\footerl': 'footer', '\\footerr': 'footer', '\\footerf': 'footer',
        }
        ...
        def flush_text(end_pos: int):
            ...
            if text:
                ...
                # Add type if we're in a header/footer context
                if hf_context:
                    stream_data["type"] = hf_context[0]
                self._streams.append(stream_data)
            text_parts = []
            char_start = end_pos
        ...
        special_keywords = {
            r'\footnote': 'footnote',
            r'\header': 'header',
            r'\headerl': 'header',  # Left page header
            r'\headerr': 'header',  # Right page header
            r'\headerf': 'header',  # First page header
            r'\footer': 'footer',
            r'\footerl': 'footer',  # Left page footer
            r'\footerr': 'footer',  # Right page footer
            r'\footerf': 'footer',  # First page footer
            r'\pict': 'pict'
        }

        # Skip these sections entirely (metadata, not content)
        skip_sections = [
            r'{\fonttbl', r'{\colortbl', r'{\stylesheet', r'{\info',
            r'{\listtable', r'{\listoverridetable', r'{\*\generator',
            r'{\*\rsidtbl', r'{\*\pgptbl', r'{\mmathPr',
            r'{\shprslt',  # Shape result - contains fallback image binary data
            # NOTE: Headers/footers are NOT skipped because they can contain
            # actual document content (e.g., chapter titles as running headers)
        ]
        ...
        while i < data_len:
            ...
            for section in skip_sections:
                if data.startswith(section, i):
                    ...
                    while i < data_len and brace_level > 0:
                        # Handle escaped braces - don't count them
                        if data[i] == '\\' and i + 1 < data_len and data[i + 1] in '{}':
                ...
                for kw, typ in special_keywords.items():
                    if data.startswith(kw, i):
                        ...
                        if j < data_len and data[j] == '{':
                            ...
                            # Not a group ({\keyword ...} structure), just skip the keyword
                            # But if it's a header/footer keyword, set the context
                            if typ in ('header', 'footer'):
                                hf_context = (typ, len(stack))
                            i = j
                            special_type = typ
                            break
                if special_type:
            ...
            # These are optional destinations that can be safely skipped
            ...
            if c == '{':
                ...
                if stack:
                    ...
                    # Clear header/footer context if we're exiting that level
                    if hf_context and len(stack) < hf_context[1]:
                        hf_context = None
                i += 1
                char_start = i
            elif c == '\\':
                ...
                if m:
                    ...
                    try:
                        ...
                        # Only output ? for actual conversion errors, not for valid Unicode
                        # This should rarely happen for valid RTF files
                        text_parts.append('?')
                    i = m.end()
                    continue
                ...
                m = _RE_HEX_CHAR.match(data, i)
                if m:
                    text_parts.append(bytes.fromhex(m.group(1)).decode('cp1252'))
                    i = m.end()
                    continue
                ...
                # Check for header/footer keywords and set context
                for hf_kw, hf_type in hf_keywords.items():
                    if data.startswith(hf_kw, i):
                        # Check keyword is complete (not followed by alphanumeric)
                        kw_end = i + len(hf_kw)
                        if kw_end >= data_len or not data[kw_end].isalnum():
                            hf_context = (hf_type, len(stack))
                            i = kw_end
                            break
                else:
